...
def populate_network(network, weights):
    ...
    fc1 = network.add_fully_connected(
        input=pool2.get_output(0), num_outputs=500, kernel=fc1_w, bias=fc1_b
    )
    ...
    fc2 = network.add_fully_connected(
        relu1.get_output(0), ModelData.OUTPUT_SIZE, fc2_w, fc2_b
    )
...
def check_output(engine, inputs, outputs, bindings, stream, mnist_model):
    ...
    with build_engine(weights) as engine:
        ...
        with trt.Refitter(engine, TRT_LOGGER) as refitter:
            ...
            refitter.set_weights(
                "conv_1", trt.WeightsRole.KERNEL, weights["conv1.weight"].numpy()
            )
            refitter.set_weights(
                "conv_1", trt.WeightsRole.BIAS, weights["conv1.bias"].numpy()
            )
            ...
            assert (
                len(missingLayers) == 0
            ), "Refitter found missing weights. Call set_weights() for all missing weights"
        ...
        assert check_output(engine, inputs, outputs, bindings, stream, mnist_model)
