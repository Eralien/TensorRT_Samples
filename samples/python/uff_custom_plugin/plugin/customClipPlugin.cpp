...
const char* ClipPlugin::getPluginType() const { return CLIP_PLUGIN_NAME; }

const char* ClipPlugin::getPluginVersion() const { return CLIP_PLUGIN_VERSION; }

int ClipPlugin::getNbOutputs() const { return 1; }

Dims ClipPlugin::getOutputDimensions(int index, const Dims* inputs,
                                     int nbInputDims) {
  // Validate input arguments
  assert(nbInputDims == 1);
  assert(index == 0);

  // Clipping doesn't change input dimension, so output Dims will be the same as
  // input Dims
  return *inputs;
}

int ClipPlugin::initialize() { return 0; }

int ClipPlugin::enqueue(int batchSize, const void* const* inputs,
  ...
  status = clipInference(stream, mInputVolume * batchSize, mClipMin, mClipMax,
                         inputs[0], output);

  return status;
}

size_t ClipPlugin::getSerializationSize() const { return 2 * sizeof(float); }
...
void ClipPlugin::configureWithFormat(const Dims* inputs, int nbInputs,
...
ClipPluginCreator::ClipPluginCreator() {
  ...
  mPluginAttributes.emplace_back(
      PluginField("clipMin", nullptr, PluginFieldType::kFLOAT32, 1));
  mPluginAttributes.emplace_back(
      PluginField("clipMax", nullptr, PluginFieldType::kFLOAT32, 1));
...
const PluginFieldCollection* ClipPluginCreator::getFieldNames() { return &mFC; }

IPluginV2* ClipPluginCreator::createPlugin(const char* name,
...
IPluginV2* ClipPluginCreator::deserializePlugin(const char* name,
