...
try:
    ...
    handle = ctypes.CDLL(os.path.join("build", "libfcplugin.so"))
except ImportError as err:
    raise ImportError(
        """ERROR: Failed to import module ({})
Please build the FullyConnected sample plugin.
For more information, see the included README.md
Note that Python 2 requires the presence of `__init__.py` in the build folder""".format(
            err
        )
    )
...
def build_engine(deploy_file, model_file):
    with trt.Builder(
        ...
        model_tensors = parser.parse(
            deploy=deploy_file, model=model_file, network=network, dtype=ModelData.DTYPE
        )
...
def main():
    ...
    data_paths, [deploy_file, model_file, mean_proto] = common.find_sample_data(
        description="Runs an MNIST network using a Caffe model file",
        subfolder="mnist",
        find_files=["mnist.prototxt", "mnist.caffemodel", "mnist_mean.binaryproto"],
    )

    # Cache the engine in a temporary directory.
    engine_path = os.path.join(tempfile.gettempdir(), "mnist.engine")
    with get_engine(
        deploy_file, model_file, engine_path
