...
nvinfer1::Weights copyToDevice(const void* hostData, int count) {
  ...
  CHECK(cudaMemcpy(deviceData, hostData, count * sizeof(float),
...
nvinfer1::Weights makeDeviceCopy(nvinfer1::Weights deviceWeights) {
  ...
  CHECK(cudaMemcpy(deviceData, deviceWeights.values,
                   deviceWeights.count * sizeof(float),
...
int copyFromDevice(char* hostBuffer, nvinfer1::Weights deviceWeights) {
  *reinterpret_cast<int*>(hostBuffer) = deviceWeights.count;
  CHECK(cudaMemcpy(hostBuffer + sizeof(int), deviceWeights.values,
                   deviceWeights.count * sizeof(float),
...
// In this simple case we're going to infer the number of output channels from
// the bias weights.
// The knowledge that the kernel weights are weights[0] and the bias weights are
// weights[1] was
...
FCPlugin::FCPlugin(const nvinfer1::Weights& deviceKernel,
                   const nvinfer1::Weights& deviceBias) {
...
int FCPlugin::getNbOutputs() const { return 1; }
...
size_t FCPlugin::getWorkspaceSize(int maxBatchSize) const { return 0; }

int FCPlugin::enqueue(int batchSize, const void* const* inputs, void** outputs,
  ...
  CHECK(cublasSgemm(mCublas, CUBLAS_OP_T, CUBLAS_OP_N, nbOutputChannels,
                    batchSize, nbInputChannels, &kONE,
                    reinterpret_cast<const float*>(mKernelWeights.values),
                    nbInputChannels, reinterpret_cast<const float*>(inputs[0]),
                    nbInputChannels, &kZERO,
                    reinterpret_cast<float*>(outputs[0]), nbOutputChannels));
  // Add bias.
  CHECK(cudnnSetTensor4dDescriptor(mSrcDescriptor, CUDNN_TENSOR_NCHW,
                                   CUDNN_DATA_FLOAT, 1, nbOutputChannels, 1,
                                   1));
  CHECK(cudnnSetTensor4dDescriptor(mDstDescriptor, CUDNN_TENSOR_NCHW,
                                   CUDNN_DATA_FLOAT, batchSize,
                                   nbOutputChannels, 1, 1));
  CHECK(cudnnAddTensor(mCudnn, &kONE, mSrcDescriptor, mBiasWeights.values,
                       &kONE, mDstDescriptor, outputs[0]));
  return 0;
}

size_t FCPlugin::getSerializationSize() const {
  return sizeof(int) * 2 + mKernelWeights.count * sizeof(float) +
...
const char* FCPlugin::getPluginType() const { return FC_PLUGIN_NAME; }

const char* FCPlugin::getPluginVersion() const { return FC_PLUGIN_VERSION; }

void FCPlugin::destroy() { delete this; }
...
DataType FCPlugin::getOutputDataType(int index,
                                     const nvinfer1::DataType* inputTypes,
                                     int nbInputs) const {
  return DataType::kFLOAT;
}

bool FCPlugin::isOutputBroadcastAcrossBatch(int outputIndex,
...
void FCPlugin::configurePlugin(const Dims* inputDims, int nbInputs,
  ...
  assert(mKernelWeights.count ==
         inputDims[0].d[0] * inputDims[0].d[1] * inputDims[0].d[2] *
             mBiasWeights.count);
}

void FCPlugin::attachToContext(cudnnContext* cudnnContext,
...
const char* FCPluginCreator::getPluginName() const { return FC_PLUGIN_NAME; }

const char* FCPluginCreator::getPluginVersion() const {
  return FC_PLUGIN_VERSION;
}

const PluginFieldCollection* FCPluginCreator::getFieldNames() { return &mFC; }

IPluginV2Ext* FCPluginCreator::createPlugin(const char* name,
...
IPluginV2Ext* FCPluginCreator::deserializePlugin(const char* name,
