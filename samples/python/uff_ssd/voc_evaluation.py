...
def produce_tensorrt_detections(
    detection_files, trt_inference_wrapper, max_batch_size, image_numbers, image_path
):
    ...
    for idx in range(0, len(image_numbers), max_batch_size):
        ...
        for img_idx, img_number in enumerate(imgs):
            img_predictions_start_idx = (
                prediction_fields * keep_count[img_idx] * img_idx
            )
            for det in range(int(keep_count[img_idx])):
                ...
                if confidence > 0.0:
                    ...
                    if label_name:
                        ...
                        detection = Detection(
                            img_number, confidence, xmin, ymin, xmax, ymax,
                        )
                        detection.write_to_file(det_file)


def produce_tensorflow_detections(
    detection_files, tf_inference_wrapper, batch_size, image_numbers, image_path
):
...
def should_skip_inference(parsed_args):
    ...
    for voc_class in VOC_CLASSES:
        voc_class_detection_file = os.path.join(
            parsed_args["results_dir"], "det_test_{}.txt".format(voc_class)
        )
        if (
            os.path.exists(voc_class_detection_file)
            and not parsed_args["force_inference"]
        ):
            continue
        else:
    ...
    if skip_inference:
        print(
            "Model detections present - skipping inference. To avoid this, use -f flag."
        )
...
def parse_commandline_arguments():
    ...
    parser.add_argument("-voc", "--voc_dir", help="VOC2007 root directory")
    ...
    if args.inference_backend == "tensorrt":
        ...
        trt_engine_path = PATHS.get_engine_path(
            trt_engine_datatype, args.max_batch_size
        )
