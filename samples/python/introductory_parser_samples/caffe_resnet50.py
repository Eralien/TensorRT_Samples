...
def allocate_buffers(engine):
    ...
    h_input = cuda.pagelocked_empty(
        trt.volume(engine.get_binding_shape(0)), dtype=trt.nptype(ModelData.DTYPE)
    )
    h_output = cuda.pagelocked_empty(
        trt.volume(engine.get_binding_shape(1)), dtype=trt.nptype(ModelData.DTYPE)
    )
...
def main():
    ...
    with build_engine_caffe(caffe_model_file, caffe_deploy_file) as engine:
        ...
        with engine.create_execution_context() as context:
            ...
            if (
                "_".join(pred.split())
                in os.path.splitext(os.path.basename(test_case))[0]
            ):
