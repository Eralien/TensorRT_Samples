...
//!
//! SampleMLP.cpp
//! This file contains the implementation of the MLP sample. It creates the
//! network
//! for MNIST classification using the API.
//! It can be run with the following command line:
//! Command: ./sample_mlp [-h or --help] [-d=/path/to/data/dir or
//! --datadir=/path/to/data/dir] [--useDLACore=<int>]
//!
...
//!
//! \brief The SampleMLPParams structure groups the additional parameters
//! required by
//!         the MLP sample.
//!
...
class SampleMLP {
 ...
 private:
  ...
  //!
  std::map<std::string, std::pair<nvinfer1::Dims, nvinfer1::Weights>>
  loadWeights(const std::string& file);
  ...
  nvinfer1::ILayer* addMLPLayer(nvinfer1::INetworkDefinition* network,
                                ...
                                int32_t hiddenSize, nvinfer1::Weights wts,
                                nvinfer1::Weights bias,
                                nvinfer1::ActivationType actType, int idx);
};
...
//! engine
//!
//! \details This function creates the MLP network by using the API to create a
//! model and builds
...
//! \brief Uses the API to create the MLP Network
//!
//! \param network Pointer to the network that will be populated with the MLP
//! network
//!
...
bool SampleMLP::constructNetwork(
  ...
  auto input = network->addInput(
      mParams.inputTensorNames[0].c_str(), nvinfer1::DataType::kFLOAT,
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    auto mlpLayer = addMLPLayer(network.get(), *input, 256,
                                mWeightMap[weightStr.str()].second,
                                mWeightMap[biasStr.str()].second,
                                nvinfer1::ActivationType::kSIGMOID, i);
  ...
  auto finalLayer = addMLPLayer(network.get(), *input, mParams.outputSize,
                                mWeightMap["outputWeights"].second,
                                mWeightMap["outputBias"].second,
                                nvinfer1::ActivationType::kSIGMOID, -1);
  assert(finalLayer != nullptr);
  // Run topK to get the final result
  auto topK = network->addTopK(*finalLayer->getOutput(0),
                               nvinfer1::TopKOperation::kMAX, 1, 0x1);
  assert(topK != nullptr);
...
bool SampleMLP::processInput(const samplesCommon::BufferManager& buffers) {
  ...
  readPGMFile(locateFile(std::to_string(mNumber) + ".pgm", mParams.dataDirs),
              fileData.data(), mParams.inputH, mParams.inputW);
...
bool SampleMLP::verifyOutput(const samplesCommon::BufferManager& buffers) {
  int idx =
      static_cast<int*>(buffers.getHostBuffer(mParams.outputTensorNames[0]))[0];
  bool pass = (idx == mNumber);
  if (pass) {
    gLogInfo << "Algorithm chose " << idx << std::endl;
  } else {
    gLogInfo << "Algorithm chose " << idx << " but expected " << mNumber << "."
             << std::endl;
  }
...
//!
std::map<std::string, std::pair<nvinfer1::Dims, nvinfer1::Weights>>
SampleMLP::loadWeights(const std::string& file) {
  ...
  while (count--) {
    ...
    wt.second.count = std::accumulate(wt.first.d, wt.first.d + wt.first.nbDims,
...
nvinfer1::Dims SampleMLP::loadShape(std::ifstream& input) {
  ...
  std::for_each(shapeDim.begin(), shapeDim.end(), [&](std::string& val) {
...
//!
//! \details
//!     The addMLPLayer function is a simple helper function that creates the
//!     combination required for an
//!     MLP layer. By replacing the implementation of this sequence with various
//!     implementations, then
//!     then it can be shown how TensorRT optimizations those layer sequences.
//!
nvinfer1::ILayer* SampleMLP::addMLPLayer(
    nvinfer1::INetworkDefinition* network, nvinfer1::ITensor& inputTensor,
    int32_t hiddenSize, nvinfer1::Weights wts, nvinfer1::Weights bias,
    nvinfer1::ActivationType actType, int idx) {
  std::string baseName("MLP Layer" +
                       (idx == -1 ? "Output" : std::to_string(idx)));
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_mlp [-h or --help] [-d or --datadir=<path to "
               "data directory>] [--useDLACore=<int>]"
            << std::endl;
...
int main(int argc, char** argv) {
  ...
  gLogInfo << "Building and running a GPU inference engine for MLP"
           << std::endl;
