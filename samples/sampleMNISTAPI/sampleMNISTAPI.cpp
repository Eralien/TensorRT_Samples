...
//!
//! sampleMNISTAPI.cpp
//! This file contains the implementation of the MNIST API sample. It creates
//! the network
//! for MNIST classification using the API.
//! It can be run with the following command line:
//! Command: ./sample_mnist_api [-h or --help] [-d=/path/to/data/dir or
...
//!
//! \brief The SampleMNISTAPIParams structure groups the additional parameters
//! required by
...
//! engine
//!
//! \details This function creates the MNIST network by using the API to create
//! a model and builds
...
bool SampleMNISTAPI::constructNetwork(
  ...
  ITensor* data =
      network->addInput(mParams.inputTensorNames[0].c_str(), DataType::kFLOAT,
  ...
  IScaleLayer* scale_1 =
      network->addScale(*data, ScaleMode::kUNIFORM, shift, scale, power);
  assert(scale_1);

  // Add convolution layer with 20 outputs and a 5x5 filter.
  IConvolutionLayer* conv1 = network->addConvolutionNd(
      *scale_1->getOutput(0), 20, Dims{2, {5, 5}, {}},
  ...
  IFullyConnectedLayer* ip1 = network->addFullyConnected(
      *pool2->getOutput(0), 500, mWeightMap["ip1filter"],
      mWeightMap["ip1bias"]);
  assert(ip1);

  // Add activation layer using the ReLU algorithm.
  IActivationLayer* relu1 =
      network->addActivation(*ip1->getOutput(0), ActivationType::kRELU);
  assert(relu1);

  // Add second fully connected layer with 20 outputs.
  IFullyConnectedLayer* ip2 = network->addFullyConnected(
      *relu1->getOutput(0), mParams.outputSize, mWeightMap["ip2filter"],
      mWeightMap["ip2bias"]);
  assert(ip2);
...
bool SampleMNISTAPI::processInput(const samplesCommon::BufferManager& buffers) {
  ...
  auto meanBlob = SampleUniquePtr<nvcaffeparser1::IBinaryProtoBlob>(
      parser->parseBinaryProto(
          locateFile(mParams.mnistMeansProto, mParams.dataDirs).c_str()));
  if (!meanBlob) {
    return false;
  }
...
//!
std::map<std::string, nvinfer1::Weights> SampleMNISTAPI::loadWeights(
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_mnist_api [-h or --help] [-d or "
...
int main(int argc, char** argv) {
  ...
  gLogInfo << "Building and running a GPU inference engine for MNIST API"
           << std::endl;
