...
template <>
#if CUDA_VERSION < 10000
inline void fillBuffer<half_float::half>(void* buffer, int volume,
                                         half_float::half min,
                                         half_float::half max)
#else
...
template <typename T>
inline void dumpBuffer(const void* buffer, int volume,
                       const std::string& separator, std::ostream& os) {
...
struct Binding {
  ...
  void fill() {
    switch (dataType) {
      ...
      case nvinfer1::DataType::kHALF: {
#if CUDA_VERSION < 10000
        fillBuffer<half_float::half>(buffer.getHostBuffer(), volume,
  ...
  void dump(std::ostream& os, const std::string separator = " ") const {
    switch (dataType) {
      ...
      case nvinfer1::DataType::kHALF: {
#if CUDA_VERSION < 10000
        dumpBuffer<half_float::half>(buffer.getHostBuffer(), volume, separator,
                                     os);
#else
...
class Bindings {
 public:
  void addBinding(int b, const std::string& name, bool isInput, int volume,
  ...
  void dumpBindingValues(int binding, std::ostream& os,
  ...
  std::unordered_map<std::string, int> getBindings(
      bool (*predicate)(const Binding& b)) const {
