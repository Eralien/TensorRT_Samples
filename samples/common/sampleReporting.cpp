...
//!
template <typename T>
float findPercentile(float percentage,
...
inline InferenceTime traceToTiming(const InferenceTrace& a) {
  return InferenceTime((a.inEnd - a.inStart), (a.computeEnd - a.computeStart),
                       (a.outEnd - a.outStart), (a.outEnd - a.inStart));
};

}  // namespace

void printProlog(int warmups, int timings, float warmupMs, float benchTimeMs,
                 std::ostream& os) {
...
void printTiming(const std::vector<InferenceTime>& timings, int runsPerAvg,
                 std::ostream& os) {
  int count = 0;
  InferenceTime sum;
  ...
  for (const auto& t : timings) {
    ...
    if (++count == runsPerAvg) {
      // clang off
      os << "Average on " << runsPerAvg
         << " runs - GPU latency: " << sum.compute / runsPerAvg
         << " ms - Host latency: " << sum.latency() / runsPerAvg
         << " ms (end to end " << sum.e2e / runsPerAvg << " ms)" << std::endl;
      // clang on
      count = 0;
      sum.in = 0;
      sum.compute = 0;
...
void printEpilog(std::vector<InferenceTime> timings, float walltimeMs,
                 float percentile, int queries, std::ostream& os) {
  const InferenceTime totalTime =
      std::accumulate(timings.begin(), timings.end(), InferenceTime());

  const auto getLatency = [](const InferenceTime& t) { return t.latency(); };
  const auto cmpLatency = [](const InferenceTime& a, const InferenceTime& b) {
  ...
  const auto cmpEndToEnd = [](const InferenceTime& a, const InferenceTime& b) {
    return a.e2e < b.e2e;
  };
  ...
  const float endToEndPercentile =
      findPercentile(percentile, timings, getEndToEnd);

  const auto getCompute = [](const InferenceTime& t) { return t.compute; };
  const auto cmpCompute = [](const InferenceTime& a, const InferenceTime& b) {
...
void printPerformanceReport(const std::vector<InferenceTrace>& trace,
                            const ReportingOptions& reporting, float warmupMs,
                            int queries, std::ostream& os) {
  const auto isNotWarmup = [&warmupMs](const InferenceTrace& a) {
  ...
  printProlog(warmups * queries, (trace.size() - warmups) * queries, warmupMs,
              benchTime, os);
...
//! Printed format:
//! [ value, ...]
//! value ::= { "start in" : time, "end in" : time, "start compute" : time, "end
//! compute" : time, "start out" : time,
//!             "in" : time, "compute" : time, "out" : time, "latency" : time,
//!             "end to end" : time}
//!
void exportJSONTrace(const std::vector<InferenceTrace>& trace,
  ...
  for (const auto& t : trace) {
    ...
    sep = ", ";
    // clang off
    os << "\"startInMs\" : " << t.inStart << sep << "\"endInMs\" : " << t.inEnd
       << sep << "\"startComputeMs\" : " << t.computeStart << sep
       << "\"endComputeMs\" : " << t.computeEnd << sep
       << "\"startOutMs\" : " << t.outStart << sep
       << "\"endOutMs\" : " << t.outEnd << sep << "\"inMs\" : " << it.in << sep
       << "\"computeMs\" : " << it.compute << sep << "\"outMs\" : " << it.out
       << sep << "\"latencyMs\" : " << it.latency() << sep
...
void Profiler::print(std::ostream& os) const {
  ...
  for (const auto& p : mLayers) {
    // clang off
    os << std::setw(nameLength) << p.name << std::setw(timeLength) << std::fixed
       << std::setprecision(2) << p.timeMs << std::setw(avgLength) << std::fixed
       << std::setprecision(2) << p.timeMs / mUpdatesCount
       << std::setw(percentageLength) << std::fixed << std::setprecision(1)
       << p.timeMs / totalTimeMs * 100 << std::endl;
  }
  {
    os << std::setw(nameLength) << "Total" << std::setw(timeLength)
       << std::fixed << std::setprecision(2) << totalTimeMs
       << std::setw(avgLength) << std::fixed << std::setprecision(2)
       << totalTimeMs / mUpdatesCount << std::setw(percentageLength)
...
void dumpInputs(const nvinfer1::IExecutionContext& context,
                const Bindings& bindings, std::ostream& os) {
...
void exportJSONOutput(const nvinfer1::IExecutionContext& context,
                      const Bindings& bindings, const std::string& fileName) {
  std::ofstream os(fileName, std::ofstream::trunc);
  std::string sep = "  ";
