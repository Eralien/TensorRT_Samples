  ...
  switch (model.baseModel.format) {
    ...
    case ModelFormat::kUFF: {
      ...
      for (const auto& s : model.uffInputs.inputs) {
        if (!parser.uffParser->registerInput(s.first.c_str(), s.second,
                                             model.uffInputs.NHWC
                                                 ? UffInputOrder::kNHWC
                                                 : UffInputOrder::kNCHW)) {
...
class RndInt8Calibrator : public nvinfer1::IInt8EntropyCalibrator2 {
 public:
  RndInt8Calibrator(int batches, const std::string& cacheFile,
...
RndInt8Calibrator::RndInt8Calibrator(int batches, const std::string& cacheFile,
  ...
  for (int i = 0; i < network.getNbInputs(); i++) {
    ...
    cudaCheck(cudaMemcpy(data, rnd_data.data(), elemCount * sizeof(float),
...
void setTensorScales(const INetworkDefinition& network, float inScales = 2.0f,
...
ICudaEngine* networkToEngine(const BuildOptions& build,
      ...
      if (isDynamicInput) {
        ...
        if (shape == build.shapes.end()) {
          ...
          if (input->isShapeTensor()) {
            ...
            std::transform(dims.d, dims.d + dims.nbDims, staticDims.d,
                           [&DEFAULT_DIMENSION](int dim) {
        ...
        if (input->isShapeTensor()) {
          ...
          profile->setShapeValues(input->getName(), OptProfileSelector::kMIN,
                                  profileDims.d, profileDims.nbDims);
  ...
  if ((build.int8 && build.calibration.empty()) || int8IO) {
    ...
    config->setInt8Calibrator(
        new RndInt8Calibrator(1, build.calibration, network, err));
  }
...
ICudaEngine* modelToEngine(const ModelOptions& model, const BuildOptions& build,
...
TrtUniquePtr<nvinfer1::ICudaEngine> getEngine(const ModelOptions& model,
