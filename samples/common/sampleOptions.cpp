...
inline std::vector<std::string> splitToStringVec(const std::string& option,
                                                 char separator) {
...
template <>
inline nvinfer1::DataType stringToValue<nvinfer1::DataType>(
...
template <>
inline nvinfer1::TensorFormats stringToValue<nvinfer1::TensorFormats>(
    const std::string& option) {
...
template <typename T>
inline void splitInsertKeyValue(const std::vector<std::string>& kvList,
                                T& map) {
...
template <typename T>
inline bool checkEraseOption(Arguments& arguments, const std::string& option,
                             T& value) {
...
template <typename T>
inline bool checkEraseRepeatedOption(Arguments& arguments,
                                     const std::string& option,
                                     std::vector<T>& values) {
  ...
  auto addValue = [&values](Arguments::value_type& value) {
    values.emplace_back(stringToValue<T>(value.second));
  };
...
void insertShapes(std::unordered_map<std::string, ShapeRange>& shapes,
                  const std::string& name, const nvinfer1::Dims& dims) {
  std::pair<std::string, ShapeRange> profile;
  profile.first = name;
  profile.second[static_cast<size_t>(nvinfer1::OptProfileSelector::kMIN)] =
      dims;
  profile.second[static_cast<size_t>(nvinfer1::OptProfileSelector::kOPT)] =
      dims;
  profile.second[static_cast<size_t>(nvinfer1::OptProfileSelector::kMAX)] =
      dims;
...
void BuildOptions::parse(Arguments& arguments) {
  ...
  auto getShapes = [&arguments](
      std::unordered_map<std::string, ShapeRange>& shapes, const char* argument,
  ...
  if (explicitBatch && batch) {
    throw std::invalid_argument(
        "Explicit batch or dynamic shapes enabled with implicit maxBatch " +
        std::to_string(batch));
  }
...
void InferenceOptions::parse(Arguments& arguments) {
  ...
  if (!shapes.empty() && batchOpt) {
    throw std::invalid_argument(
        "Explicit batch or dynamic shapes enabled with implicit batch " +
...
void AllOptions::parse(Arguments& arguments) {
  ...
  if ((!build.maxBatch && inference.batch && inference.batch != defaultBatch &&
       !build.shapes.empty()) ||
      (build.maxBatch && build.maxBatch != defaultMaxBatch &&
       !inference.batch)) {
    // If either has selected implict batch and the other has selected explicit
    // batch
    throw std::invalid_argument(
  ...
  if (build.shapes.empty() && !inference.shapes.empty()) {
    ...
    if (!build.shapes.empty() && inference.shapes.empty()) {
      for (auto& s : build.shapes) {
        inference.shapes.insert(
            {s.first, s.second[static_cast<size_t>(
  ...
  if (build.maxBatch && inference.batch) {
    // For implicit batch, check for compatibility and if --maxBatch is not
    // given and inference batch is greater
  ...
  if (!helps) {
    ...
    if (!build.load && !build.maxBatch &&
        model.baseModel.format != ModelFormat::kONNX) {
      throw std::invalid_argument(
          "Explicit batch size not supported for Caffe and Uff models");
    }
    if (build.safe && system.DLACore >= 0) {
      auto checkSafeDLAFormats = [](const std::vector<IOFormat>& fmt) {
        return fmt.empty()
                   ? false
                   : std::all_of(
                         fmt.begin(), fmt.end(), [](const IOFormat& pair) {
                           bool supported{false};
                           supported |=
                               pair.first == nvinfer1::DataType::kINT8 &&
                               pair.second ==
                                   1U << static_cast<int>(
                                       nvinfer1::TensorFormat::kCHW32);
                           supported |=
                               pair.first == nvinfer1::DataType::kHALF &&
                               pair.second ==
                                   1U << static_cast<int>(
      ...
      if (!checkSafeDLAFormats(build.inputFormats) ||
          !checkSafeDLAFormats(build.inputFormats)) {
        throw std::invalid_argument(
            "I/O formats for safe DLA capability are restricted to fp16:chw16 "
            "or int8:chw32");
      }
      if (system.fallback) {
        throw std::invalid_argument(
