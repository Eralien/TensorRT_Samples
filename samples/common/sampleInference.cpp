...
void setUpInference(InferenceEnvironment& iEnv,
      ...
      if (isDynamicInput) {
        ...
        if (shape == inference.shapes.end()) {
          ...
          if (iEnv.engine->isShapeBinding(b)) {
            ...
            std::fill(staticDims.d, staticDims.d + staticDims.nbDims,
                      DEFAULT_DIMENSION);
          } else {
            staticDims.nbDims = dims.nbDims;
            std::transform(
                dims.d, dims.d + dims.nbDims, staticDims.d,
                [&](int dim) { return dim > 0 ? dim : DEFAULT_DIMENSION; });
          }
          gLogWarning << "Dynamic dimensions required for input: "
...
class EnqueueImplicit {
 public:
  ...
  void operator()(nvinfer1::IExecutionContext& context, void** buffers,
                  TrtCudaStream& stream) const {
...
using EnqueueFunction =
    std::function<void(nvinfer1::IExecutionContext&, void**, TrtCudaStream&)>;
...
using MultiStream =
    std::array<TrtCudaStream, static_cast<int>(StreamType::kNUM)>;

using MultiEvent = std::array<std::unique_ptr<TrtCudaEvent>,
                              static_cast<int>(EventType::kNUM)>;
...
class Iteration {
 public:
  Iteration(int id, bool overlap, bool spin,
            nvinfer1::IExecutionContext& context, Bindings& bindings,
            EnqueueFunction enqueue)
      : mContext(context),
        mBindings(bindings),
        mEnqueue(enqueue),
        mStreamId(id),
  ...
  void query() {
    ...
    mEnqueue(mContext, mBindings.getDeviceBuffers(),
             getStream(StreamType::kCOMPUTE));
  ...
  void wait(TrtCudaEvent& start) { getStream(StreamType::kINPUT).wait(start); }
 ...
 private:
  ...
  InferenceTrace getTrace(const TrtCudaEvent& start) {
    return InferenceTrace(mStreamId, getEvent(EventType::kINPUT_S) - start,
                          getEvent(EventType::kINPUT_E) - start,
                          getEvent(EventType::kCOMPUTE_S) - start,
                          getEvent(EventType::kCOMPUTE_E) - start,
                          getEvent(EventType::kOUTPUT_S) - start,
                          getEvent(EventType::kOUTPUT_E) - start);
  }
...
void inferenceLoop(IterationStreams& iStreams, const TrtCudaEvent& mainStart,
                   int batch, int iterations, float maxDurationMs,
                   float warmupMs, std::vector<InferenceTrace>& trace) {
  float durationMs = 0;
  int skip = 0;
...
void inferenceExecution(const InferenceOptions& inference,
  ...
  auto enqueue = inference.batch
                     ? EnqueueFunction(EnqueueImplicit(inference.batch))
                     : EnqueueFunction(EnqueueExplicit());

  IterationStreams iStreams;
  for (int s = 0; s < streams; ++s) {
    iStreams.emplace_back(new Iteration(offset + s, inference.overlap,
                                        inference.spin, *iEnv.context[offset],
                                        *iEnv.bindings[offset], enqueue));
  }
  ...
  inferenceLoop(iStreams, sync.mainStart, inference.batch, inference.iterations,
                durationMs, warmupMs, localTrace);
...
inline std::thread makeThread(const InferenceOptions& inference,
                              InferenceEnvironment& iEnv, SyncStruct& sync,
                              int thread, int streamsPerThread,
                              std::vector<InferenceTrace>& trace) {
  return std::thread(inferenceExecution, std::cref(inference), std::ref(iEnv),
                     std::ref(sync), thread, streamsPerThread, std::ref(trace));
}

}  // namespace

void runInference(const InferenceOptions& inference, InferenceEnvironment& iEnv,
                  std::vector<InferenceTrace>& trace) {
  trace.resize(0);
  ...
  for (int t = 0; t < threadsNum; ++t) {
    threads.emplace_back(
        makeThread(inference, iEnv, sync, t, streamsPerThread, trace));
  }
  for (auto& th : threads) {
    th.join();
  }
