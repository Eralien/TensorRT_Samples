...
class SampleConfig : public nvonnxparser::IOnnxConfig {
 ...
 public:
  ...
  nvinfer1::DataType getModelDtype() const { return mModelDtype; }
  ...
  void setLabel(int64_t label) { mLabel = label; }  //!<  set the Label
  int64_t getLabel() const { return mLabel; }       //!<  get the Label
  ...
  void setReferenceFileName(
      const char* referenceFilename)  //!<  set reference file name
  {
  ...
  const char* getOutputFileName()
      const  //!<  specifies the file to save the results
  {
    return mOutputFilename.c_str();
  }
  void setOutputFileName(
      const char* outputFilename)  //!<  get the output file name
  {
  ...
  const char* getCalibrationFileName() const {
    return mCalibrationFilename.c_str();
     ...
     //! calibration
  void setCalibrationFileName(const char* calibrationFilename)  //!<  get the
                                                                //! int 8
  //! calibration
  //! list file
  //! name
  ...
  float getFailurePercentage() const { return mFailurePercentage; }

  void setFailurePercentage(float f) { mFailurePercentage = f; }
