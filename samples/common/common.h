...
// it might lie in
// Function will also walk back MAX_DEPTH dirs from CWD to check for such a file
// path
...
class HostMemory : public IHostMemory {
 ...
 protected:
  HostMemory(std::size_t size, DataType type) : mSize(size), mType(type) {}
  void* mData;
  std::size_t mSize;
  DataType mType;
};
...
class TypedHostMemory : public HostMemory {
 public:
  TypedHostMemory(std::size_t size) : HostMemory(size, dataType) {
...
inline std::vector<std::string> classify(
    const std::vector<std::string>& refVector, const result_vector_t& output,
    const size_t topK) {
...
template <typename T>
inline bool readASCIIFile(const std::string& fileName, const size_t size,
                          std::vector<T>& out) {
  std::ifstream infile(fileName);
  if (!infile.is_open()) {
...
template <typename T>
inline bool writeASCIIFile(const std::string& fileName,
...
// used.
// This function is just a utility to globally fill in missing scales for the
// entire network.
//
...
inline void setDummyInt8Scales(const IBuilderConfig* c, INetworkDefinition* n) {
  ...
  if (c->getFlag(BuilderFlag::kINT8)) {
    gLogWarning << "Int8 calibrator not provided. Generating dummy per tensor "
