...
//!
struct InferenceTime {
  InferenceTime(float i, float c, float o, float e)
      : in(i), compute(c), out(o), e2e(e) {}
...
//!
struct InferenceTrace {
  InferenceTrace(int s, float is, float ie, float cs, float ce, float os,
                 float oe)
      : stream(s),
        inStart(is),
        inEnd(ie),
...
inline InferenceTime operator+(const InferenceTime& a, const InferenceTime& b) {
  return InferenceTime(a.in + b.in, a.compute + b.compute, a.out + b.out,
                       a.e2e + b.e2e);
}
...
//!
void printProlog(int warmups, int timings, float warmupMs, float walltime,
                 std::ostream& os);
...
//!
void printEpilog(std::vector<InferenceTime> timings, float percentile,
