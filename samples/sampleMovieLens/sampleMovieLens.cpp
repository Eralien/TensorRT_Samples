...
//!
//! sampleMovieLens.cpp
//! This file contains the implementation of the MovieLens sample. It creates
//! the network using
//! the MLP NCF Uff model.
//! It can be run with the following command line:
//! Command: ./sample_movielens [-h or --help] [-b NUM_USERS]
...
// The OutputParams struct holds intermediate/final outputs generated by the
...
//!
//! \brief The SampleMovieLensParams structure groups the additional parameters
//! required by
//!         the MovieLens sample.
//!
struct SampleMovieLensParams : public samplesCommon::UffSampleParams {
  int32_t embeddingVecSize;
  int32_t numUsers;    // Total number of users. Should be equal to ratings file
                       // users count.
...
class SampleMovieLens {
 ...
 public:
  SampleMovieLens(const SampleMovieLensParams& params) : mParams(params) {}
 ...
 private:
  //!
  //! \brief Parses a Uff model for a MLP NCF model, creates a TensorRT network,
  ...
  //!
  //! \brief Copies a batch of input data from SampleMovieLensParams into
  ...
  //!        dataset .csv file, and populates the corresponding ground truth
  //!        data struct
  //!
  void readInputSample(std::ifstream& file, OutputParams& outParams,
                       std::string line);

  //!
  //! \brief Parses the MovieLens dataset and populates the
  //! SampleMovieLensParams data structure
  //!
  ...
  //!
  //! \brief Verifies the inference output with ground truth and logs the
  //! results
  //!
  bool verifyOutput(uint32_t* userInputPtr, uint32_t* /*itemInputPtr*/,
                    uint32_t* topKItemNumberPtr, float* topKItemProbPtr);
...
//! and builds the engine that will be used to generate recommendations
//! (mEngine)
//!
...
//! and builds a TensorRT engine.
//!
void SampleMovieLens::constructNetwork(
  ...
  parser->registerInput(mParams.inputTensorNames[0].c_str(), inputIndices,
                        nvuffparser::UffInputOrder::kNCHW);
  parser->registerInput(mParams.inputTensorNames[1].c_str(), inputIndices,
                        nvuffparser::UffInputOrder::kNCHW);
  ...
  reshapeLayer->setReshapeDimensions(
      nvinfer1::Dims3(1, mParams.numMoviesPerUser, 1));
  assert(reshapeLayer != nullptr);

  // Apply TopK layer to retrieve item probabilities and corresponding index
  // number.
  auto topK =
      network->addTopK(*reshapeLayer->getOutput(0),
                       nvinfer1::TopKOperation::kMAX, mParams.topKMovies, 0x2);
  assert(topK != nullptr);
...
//! allocates the buffer, sets inputs, executes the engine, and verifies the
//! output.
//!
bool SampleMovieLens::infer() {
  ...
  timer.stop();
  gLogInfo << "Done execution. Duration : " << timer.microseconds()
  ...
  float* topKItemProb =
      static_cast<float*>(buffers.getHostBuffer(mParams.outputTensorNames[1]));
  uint32_t* topKItemNumber = static_cast<uint32_t*>(
      buffers.getHostBuffer(mParams.outputTensorNames[2]));

  uint32_t* userInput = static_cast<uint32_t*>(
      buffers.getHostBuffer(mParams.inputTensorNames[0]));
  uint32_t* itemInput = static_cast<uint32_t*>(
      buffers.getHostBuffer(mParams.inputTensorNames[1]));

  return SampleMovieLens::verifyOutput(userInput, itemInput, topKItemNumber,
                                       topKItemProb);
}

//!
//! \brief Copies a batch of input data from SampleMovieLensParams into managed
//! input buffers
//!
bool SampleMovieLens::processInput(
...
//!
//! \brief Extracts needed dataset values for a single user in the MovieLens,
//!        dataset .csv file, and populates the corresponding ground truth data
//!        struct
//!
void SampleMovieLens::readInputSample(std::ifstream& file,
  ...
  outParams.expectedPredictedMaxRatingItem =
      std::stoi(readNextLine(file, delim));
...
//!
//! \brief Parses the MovieLens dataset and populates the SampleMovieLensParams
//! data structure
//!
void SampleMovieLens::parseMovieLensData() {
  ...
  while (std::getline(file, line) && userIdx < mParams.batchSize) {
    ...
    mParams.userToExpectedItemProbMap[userIdx] =
        std::move(outParams.itemProbPairVec);
...
void SampleMovieLens::printOutputParams(OutputParams& outParams) {
  gLogVerbose << "User Id                            :   " << outParams.userId
              << std::endl;
  gLogVerbose << "Expected Predicted Max Rating Item :   "
              << outParams.expectedPredictedMaxRatingItem << std::endl;
  gLogVerbose << "Expected Predicted Max Rating Prob :   "
              << outParams.expectedPredictedMaxRatingItemProb << std::endl;
  gLogVerbose << "Total TopK Items : " << outParams.itemProbPairVec.size()
              << std::endl;
  for (unsigned int i = 0; i < outParams.itemProbPairVec.size(); ++i) {
    gLogVerbose << outParams.itemProbPairVec.at(i).first << " : "
                << outParams.itemProbPairVec.at(i).second << std::endl;
  }
}
...
//!
bool SampleMovieLens::verifyOutput(uint32_t* userInput, uint32_t* /*itemInput*/,
  ...
  for (int i = 0; i < mParams.batchSize; ++i) {
    ...
    int maxExpectedItem =
        mParams.userToExpectedItemProbMap.at(userIdx).at(0).first;
    int maxPredictedItem =
        mParams.userToItemsMap.at(userIdx).at(maxPredictedIdx);
    ...
    for (int k = 0; k < mParams.topKMovies; ++k) {
      ...
      float expectedProb =
          mParams.userToExpectedItemProbMap.at(userIdx).at(k).second;
  ...
  for (int i = 0; i < mParams.batchSize; ++i) {
    ...
    int maxPredictedItem =
        mParams.userToItemsMap.at(userIdx).at(maxPredictedIdx);
    gLogInfo << "| User :" << std::setw(4) << userIdx
...
//!
//! \brief Parses the command line arguments for the MovieLens sample, and
//! returns failure
//!        if arguments are incorrect
//!
bool parseSampleMovieLensArgs(SampleMovieLensArgs& args, int argc,
                              char* argv[]) {
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_movielens [-h or --help] [-b NUM_USERS] "
               "[--useDLACore=<int>] [--verbose]\n";
  std::cout << "--help          Display help information.\n";
  std::cout << "--verbose       Enable verbose prints.\n";
  std::cout << "-b NUM_USERS    Number of Users i.e. Batch Size (default "
               "numUsers==32).\n";
