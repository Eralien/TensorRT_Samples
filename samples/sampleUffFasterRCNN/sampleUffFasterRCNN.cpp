...
//! This file contains the implementation of the Uff FasterRCNN sample. It
//! creates the network using
...
class SampleUffFasterRcnn {
 ...
 private:
  ...
  void batch_inverse_transform_classifier(
      ...
      std::vector<float>& pred_probs, std::vector<int>& box_num_per_img, int N);
  ...
  //!
  void visualize_boxes(int img_num, int class_num,
...
bool SampleUffFasterRcnn::constructNetwork(
    SampleUniquePtr<nvinfer1::IBuilder>& builder,
    SampleUniquePtr<nvinfer1::INetworkDefinition>& network,
    SampleUniquePtr<nvuffparser::IUffParser>& parser) {
  parser->registerInput(
      mParams.inputNodeName.c_str(),
      DimsCHW(mParams.inputChannels, mParams.inputHeight, mParams.inputWidth),
  ...
  if (mParams.int8) {
    ...
    const nvinfer1::DimsNCHW imageDims{mParams.calBatchSize, imageC, imageH,
                                       imageW};
    ...
    calibrator.reset(new Int8EntropyCalibrator2(
        calibrationStream, 0, "UffFasterRcnn", mParams.inputNodeName.c_str()));
...
bool SampleUffFasterRcnn::processInput(
  ...
  float* hostDataBuffer =
      static_cast<float*>(buffers.getHostBuffer(mParams.inputNodeName));
    ...
    for (int c = 0; c < inputC; ++c) {
      for (unsigned j = 0, volChl = inputH * inputW; j < volChl; ++j) {
        hostDataBuffer[i * volImg + c * volChl + j] =
            float(ppms[i].buffer[j * inputC + 2 - c]) - pixelMean[c];
      }
    }
  }
...
bool SampleUffFasterRcnn::verifyOutput(
    const samplesCommon::BufferManager& buffers) {
  const int batchSize = mParams.batchSize;
  ...
  const float* out_class =
      static_cast<const float*>(buffers.getHostBuffer(mParams.outputClsName));
  const float* out_reg =
      static_cast<const float*>(buffers.getHostBuffer(mParams.outputRegName));
  const float* out_proposal = static_cast<const float*>(
      buffers.getHostBuffer(mParams.outputProposalName));
  ...
  batch_inverse_transform_classifier(out_proposal, post_nms_top_n, out_class,
                                     out_reg, pred_boxes, pred_cls_ids,
                                     pred_probs, box_num_per_img, batchSize);
  visualize_boxes(batchSize, outputClassSize, pred_boxes, pred_probs,
                  pred_cls_ids, box_num_per_img, ppms);
  return true;
}
...
void printHelpInfo() {
  ...
  std::cout
      << "--datadir[-d]           Specify path to a data directory, overriding "
      ...
      << std::endl;
  std::cout << "--useDLACore[-u]        Specify a DLA engine for layers that "
               "support DLA. "
               "Value can range from 0 to n-1, "
               "where n is the number of DLA engines on the platform."
            << std::endl;
  std::cout << "--fp16[-f]              Specify to run in fp16 mode."
            << std::endl;
  std::cout << "--int8[-i]              Specify to run in int8 mode."
            << std::endl;
  std::cout << "--inputWidth[-W]        Specify the input width of the model."
            << std::endl;
  std::cout << "--inputHeight[-H]       Specify the input height of the model."
            << std::endl;
  std::cout << "--batchSize[-B]         Specify the batch size for inference."
            << std::endl;
  std::cout << "--profile[-p]           Whether to do per-layer profiling."
            << std::endl;
  std::cout << "--repeat[-r]            Specify the repeat number to execute "
               "the TRT context,"
               " used to smooth the profiling time."
            << std::endl;
  std::cout << "--inputImages[-I]       Specify the input images for inference."
            << std::endl;
  ...
  for (int n = 0; n < N; ++n) {
    ...
    for (int i = 0; i < roi_num_per_img; ++i) {
      auto max_idx = max_index(
          classifier_cls + n * roi_num_per_img * mParams.outputClassSize +
              i * mParams.outputClassSize,
          classifier_cls + n * roi_num_per_img * mParams.outputClassSize +
              i * mParams.outputClassSize + mParams.outputClassSize);

      if (max_idx == (mParams.outputClassSize - 1) ||
          classifier_cls[n * roi_num_per_img * mParams.outputClassSize +
                         max_idx + i * mParams.outputClassSize] <
      ...
      float tx, ty, tw, th;
      //(i, 20, 4)
      tx = classifier_regr[n * roi_num_per_img * mParams.outputBboxSize +
                           i * mParams.outputBboxSize + max_idx * 4] /
           mParams.classifierRegressorStd[0];
      ty = classifier_regr[n * roi_num_per_img * mParams.outputBboxSize +
                           i * mParams.outputBboxSize + max_idx * 4 + 1] /
           mParams.classifierRegressorStd[1];
      tw = classifier_regr[n * roi_num_per_img * mParams.outputBboxSize +
                           i * mParams.outputBboxSize + max_idx * 4 + 2] /
           mParams.classifierRegressorStd[2];
      th = classifier_regr[n * roi_num_per_img * mParams.outputBboxSize +
                           i * mParams.outputBboxSize + max_idx * 4 + 3] /
           mParams.classifierRegressorStd[3];
      float y = roi_after_nms[n * roi_num_per_img * 4 + 4 * i] *
                static_cast<float>(mParams.inputHeight);
      float x = roi_after_nms[n * roi_num_per_img * 4 + 4 * i + 1] *
                static_cast<float>(mParams.inputWidth);
      float ymax = roi_after_nms[n * roi_num_per_img * 4 + 4 * i + 2] *
                   static_cast<float>(mParams.inputHeight);
      float xmax = roi_after_nms[n * roi_num_per_img * 4 + 4 * i + 3] *
                   static_cast<float>(mParams.inputWidth);
      float w = xmax - x;
      float h = ymax - y;
      float cx = x + w / 2.0f;
      ...
      float cy1 = ty * h + cy;
      float w1 =
          std::round(std::exp(static_cast<double>(tw)) * w * 0.5f) * 2.0f;
      float h1 =
          std::round(std::exp(static_cast<double>(th)) * h * 0.5f) * 2.0f;
...
std::vector<int> SampleUffFasterRcnn::nms_classifier(
    std::vector<float>& boxes_per_cls, std::vector<float>& probs_per_cls,
  ...
  for (auto i : score_index) {
    ...
    for (unsigned k = 0; k < indices.size(); ++k) {
      if (keep) {
        ...
        float overlap =
            computeIoU(&boxes_per_cls[idx * 4], &boxes_per_cls[kept_idx * 4]);
...
//! \brief Dump the detection results(bboxes) as PPM images, overlayed on
//! original image.
//!
void SampleUffFasterRcnn::visualize_boxes(int img_num, int class_num,
                                          std::vector<float>& pred_boxes,
                                          std::vector<float>& pred_probs,
                                          std::vector<int>& pred_cls_ids,
                                          std::vector<int>& box_num_per_img,
  ...
  for (int i = 0; i < img_num; ++i) {
    ...
    for (int c = 0; c < (class_num - 1); ++c) {  // skip the background
      boxes_per_cls.clear();
      probs_per_cls.clear();
      ...
      auto indices_after_nms = nms_classifier(boxes_per_cls, probs_per_cls,
                                              mParams.nmsIouThresholdClassifier,
                                              mParams.postNmsTopN);

      // Show results
      for (unsigned k = 0; k < indices_after_nms.size(); ++k) {
        int idx = indices_after_nms[k];
        std::cout << "Detected " << mParams.classNames[c] << " in "
                  << ppms[i].fileName << " with confidence "
                  << probs_per_cls[idx] * 100.0f << "% " << std::endl;
        BBox b{boxes_per_cls[idx * 4], boxes_per_cls[idx * 4 + 1],
               boxes_per_cls[idx * 4 + 2], boxes_per_cls[idx * 4 + 3]};
...
int main(int argc, char** argv) {
  ...
  auto sampleTest =
      gLogger.defineTest(gSampleName, argc, const_cast<const char**>(argv));
  gLogger.reportTestStart(sampleTest);
  SampleUffFasterRcnn sample(initializeSampleParams(args));
  gLogInfo << "Building and running a GPU inference engine for FasterRCNN"
           << std::endl;
