...
void splitStr(const char* s, std::vector<std::string>& ret, char del = ',') {
  ...
  while (std::string::npos != std::string(s + idx).find(std::string(1, del))) {
    auto s_tmp = std::string(s + idx).substr(
        0, std::string(s + idx).find(std::string(1, del)));
    ret.push_back(s_tmp);
...
bool parseFrcnnArgs(FrcnnArgs& args, int argc, char* argv[]) {
  while (1) {
    ...
    static struct option long_options[] = {
        ...
        {"inputHeight", required_argument, 0, 'H'},
        {"inputWidth", required_argument, 0, 'w'},
        {"repeat", required_argument, 0, 'r'},
        {"profile", no_argument, 0, 'p'},
        {"batchSize", required_argument, 0, 'b'},
        {"inputImages", required_argument, 0, 'I'},
        {"saveEngine", required_argument, 0, 's'},
        {"loadEngine", required_argument, 0, 'l'},
        {nullptr, 0, nullptr, 0}};
    int option_index = 0;
    arg = getopt_long(argc, argv, "hd:ifuH:W:r:pB:I:s:l:", long_options,
                      &option_index);
    ...
    switch (arg) {
      ...
      case 'H':
        args.inputHeight = std::stoi(optarg);
        break;

      case 'W':
        args.inputWidth = std::stoi(optarg);
        break;
      ...
      case 'I':
        splitStr(optarg, args.inputImages);
        break;
      case 's':
...
//! \brief resize PPM on-the-fly so that user can specify input dimensions as
//! commandline args.
//!
void resizePPM(vPPM& ppm, int target_width, int target_height) {
  ...
  for (int y = 0; y < target_height; ++y) {
    for (int x = 0; x < target_width; ++x) {
      ...
      for (int c = 0; c < 3; ++c) {
        // H, W, C ordering
        uint8_t left_top_val =
            ppm.buffer[top * (original_width * 3) + left * (3) + c];
        uint8_t right_top_val =
            ppm.buffer[top * (original_width * 3) + right * (3) + c];
        uint8_t left_bottom_val =
            ppm.buffer[bottom * (original_width * 3) + left * (3) + c];
        uint8_t right_bottom_val =
            ppm.buffer[bottom * (original_width * 3) + right * (3) + c];
        float top_lerp =
...
//! \class BatchStream
//!
//! \brief Custom BatchStream class for Faster-RCNN because we use variable
//! input dimensions and different image
//! preprocessing.
//!
class BatchStream {
 public:
    ...
    if (mListFile.empty()) {
      ...
      for (int i = 0, volImg = mDims.d[1] * mDims.d[2] * mDims.d[3];
        ...
        for (int c = 0; c < mDims.d[1]; ++c) {
          for (unsigned j = 0, volChl = mDims.d[2] * mDims.d[3]; j < volChl;
               ++j) {
...
class EntropyCalibratorImpl {
 public:
  EntropyCalibratorImpl(BatchStream& stream, int firstBatch,
...
class Int8EntropyCalibrator2 : public IInt8EntropyCalibrator2 {
 public:
  Int8EntropyCalibrator2(BatchStream& stream, int firstBatch,
