...
int main(int argc, char** argv) {
    ...
    try {
      ...
      if (!args.empty()) {
        for (const auto& arg : args) {
          gLogError << "Unknown option: " << arg.first << " " << arg.second
                    << std::endl;
        }
        failed = true;
      }
  ...
  iEnv.engine =
      getEngine(options.model, options.build, options.system, gLogError);
  if (!iEnv.engine) {
  ...
  if (options.build.safe && options.system.DLACore >= 0) {
    gLogInfo << "Safe DLA capability is detected. Please save DLA loadable "
                "with --saveEngine option, "
                "then use dla_safety_runtime to run inference with saved DLA "
                "loadable, "
  ...
  printPerformanceReport(trace, options.reporting,
                         static_cast<float>(options.inference.warmup),
                         options.inference.batch, gLogInfo);
  ...
  if (!options.reporting.exportOutput.empty()) {
    exportJSONOutput(*iEnv.context.front(), *iEnv.bindings.front(),
                     options.reporting.exportOutput);
  }
