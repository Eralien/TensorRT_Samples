...
struct Mask {
  float raw[MaskRCNNConfig::MASK_POOL_SIZE * 2 *
            MaskRCNNConfig::MASK_POOL_SIZE * 2];
};
...
template <typename T>
void resizePPM(const PPM<T>& src, PPM<T>& dst, int target_height,
               int target_width, int channel) {
  ...
  float ratio_h = static_cast<float>(original_height - 1.0f) /
                  static_cast<float>(target_height - 1.0f);
  float ratio_w = static_cast<float>(original_width - 1.0f) /
  ...
  for (int y = 0; y < target_height; ++y) {
    for (int x = 0; x < target_width; ++x) {
      ...
      int left = static_cast<int>(clip(
          std::floor(x0), 0.0f, static_cast<float>(original_width - 1.0f)));
      int top = static_cast<int>(clip(
          std::floor(y0), 0.0f, static_cast<float>(original_height - 1.0f)));
      int right = static_cast<int>(
          clip(std::ceil(x0), 0.0f, static_cast<float>(original_width - 1.0f)));
      int bottom = static_cast<int>(clip(
          std::ceil(y0), 0.0f, static_cast<float>(original_height - 1.0f)));

      for (int c = 0; c < channel; ++c) {
        // H, W, C ordering
        T left_top_val =
            src.buffer[top * (original_width * channel) + left * (channel) + c];
        T right_top_val = src.buffer[top * (original_width * channel) +
                                     right * (channel) + c];
        T left_bottom_val = src.buffer[bottom * (original_width * channel) +
                                       left * (channel) + c];
        T right_bottom_val = src.buffer[bottom * (original_width * channel) +
                                        right * (channel) + c];
        float top_lerp =
            left_top_val + (right_top_val - left_top_val) * (x0 - left);
        float bottom_lerp = left_bottom_val +
                            (right_bottom_val - left_bottom_val) * (x0 - left);
        float lerp =
            clip(std::round(top_lerp + (bottom_lerp - top_lerp) * (y0 - top)),
                 0.0f, 255.0f);
...
void padPPM(const PPM<uint8_t>& src, PPM<uint8_t>& dst, int top, int bottom,
            int left, int right) {
  ...
  for (int y = 0; y < src.h; y++) {
    for (int x = 0; x < src.w; x++) {
      for (int c = 0; c < 3; c++) {
        dst.buffer[(top + y) * dst.w * 3 + (left + x) * 3 + c] =
            src.buffer[y * src.w * 3 + x * 3 + c];
      }
    }
  }
}

void preprocessPPM(PPM<uint8_t>& src, PPM<uint8_t>& dst, int target_h,
                   int target_w) {
  ...
  padPPM(resized_ppm, dst, y_offset, input_dim - resize_h - y_offset, x_offset,
         input_dim - resize_w - x_offset);
}
...
void maskPPM(PPM<uint8_t>& image, const PPM<uint8_t>& mask, const int start_x,
             const int start_y, const std::vector<int>& color) {
  float alpha = 0.6f;
  ...
  for (int y = 0; y < mask.h; ++y) {
    for (int x = 0; x < mask.w; ++x) {
      ...
      if (mask_pixel == 1) {
        ...
        float p_r =
            static_cast<float>(image.buffer[(cur_y * image.w + cur_x) * 3]);
        float p_g =
            static_cast<float>(image.buffer[(cur_y * image.w + cur_x) * 3 + 1]);
        float p_b =
            static_cast<float>(image.buffer[(cur_y * image.w + cur_x) * 3 + 2]);

        image.buffer[(cur_y * image.w + cur_x) * 3] =
            static_cast<uint8_t>(std::max(
                0.0f, std::min(255.0f, p_r * (1 - alpha) + color[0] * alpha)));
        image.buffer[(cur_y * image.w + cur_x) * 3 + 1] =
            static_cast<uint8_t>(std::max(
                0.0f, std::min(255.0f, p_g * (1 - alpha) + color[1] * alpha)));
        image.buffer[(cur_y * image.w + cur_x) * 3 + 2] =
            static_cast<uint8_t>(std::max(
                0.0f, std::min(255.0f, p_b * (1 - alpha) + color[2] * alpha)));
      } else
        assert(mask_pixel == 0);
    }
  }
}
void addBBoxPPM(PPM<uint8_t>& ppm, const BBoxInfo& box,
                const PPM<uint8_t>& resized_mask) {
...
class SampleMaskRCNN {
 ...
 private:
  ...
  vector<MaskRCNNUtils::BBoxInfo> decodeOutput(const int imageIdx,
                                               void* detectionsHost,
                                               void* masksHost);
};
...
bool SampleMaskRCNN::build() {
  ...
  auto parser =
      SampleUniquePtr<nvuffparser::IUffParser>(nvuffparser::createUffParser());
  if (!parser) {
    return false;
  }
...
bool SampleMaskRCNN::constructNetwork(
...
bool SampleMaskRCNN::infer() {
  ...
  gLogInfo << "Run for 10 times with Batch Size " << mParams.batchSize
           << std::endl;
  gLogInfo << "Average inference time is "
           << (totalHost / 10) / mParams.batchSize << " ms/frame" << std::endl;
...
bool SampleMaskRCNN::processInput(const samplesCommon::BufferManager& buffers) {
  ...
  for (int i = 0; i < batchSize; ++i) {
    MaskRCNNUtils::readPPMFile(locateFile(imageList[i], mParams.dataDirs),
                               mOriginalPPMs[i]);
  ...
  for (int i = 0, volImg = inputC * inputH * inputW; i < mParams.batchSize;
       ++i) {
    for (int c = 0; c < inputC; ++c) {
      // The color image to input should be in RGB order
      for (unsigned j = 0, volChl = inputH * inputW; j < volChl; ++j) {
        hostDataBuffer[i * volImg + c * volChl + j] =
            float(mPPMs[i].buffer[j * inputC + c]) - pixelMean[c];
      }
    }
  }
...
vector<MaskRCNNUtils::BBoxInfo> SampleMaskRCNN::decodeOutput(
    const int imageIdx, void* detectionsHost, void* masksHost) {
  int input_dim_h = MaskRCNNConfig::IMAGE_SHAPE.d[1],
      input_dim_w = MaskRCNNConfig::IMAGE_SHAPE.d[2];
  ...
  int detectionOffset =
      samplesCommon::volume(MaskRCNNConfig::MODEL_DETECTION_SHAPE);  // (100,6)
  int maskOffset = samplesCommon::volume(
      MaskRCNNConfig::MODEL_MASK_SHAPE);  // (100, 81, 28, 28)

  MaskRCNNUtils::RawDetection* detections =
      reinterpret_cast<MaskRCNNUtils::RawDetection*>(
          (float*)detectionsHost + imageIdx * detectionOffset);
  MaskRCNNUtils::Mask* masks = reinterpret_cast<MaskRCNNUtils::Mask*>(
      (float*)masksHost + imageIdx * maskOffset);
  for (int det_id = 0; det_id < MaskRCNNConfig::DETECTION_MAX_INSTANCES;
       det_id++) {
    ...
    det.box.x1 =
        std::min(std::max((cur_det.x1 - window_x) * final_ratio_x, 0.0f),
                 (float)image_width);
    det.box.y1 =
        std::min(std::max((cur_det.y1 - window_y) * final_ratio_y, 0.0f),
                 (float)image_height);
    det.box.x2 =
        std::min(std::max((cur_det.x2 - window_x) * final_ratio_x, 0.0f),
                 (float)image_width);
    det.box.y2 =
        std::min(std::max((cur_det.y2 - window_y) * final_ratio_y, 0.0f),
                 (float)image_height);
...
bool SampleMaskRCNN::verifyOutput(const samplesCommon::BufferManager& buffers) {
  ...
  for (int p = 0; p < mParams.batchSize; ++p) {
    vector<MaskRCNNUtils::BBoxInfo> binfo =
        decodeOutput(p, detectionsHost, masksHost);
    for (size_t roi_id = 0; roi_id < binfo.size(); roi_id++) {
      const auto resized_mask = MaskRCNNUtils::resizeMask(
          binfo[roi_id], mParams.maskThreshold);  // mask threshold
      MaskRCNNUtils::addBBoxPPM(mOriginalPPMs[p], binfo[roi_id], resized_mask);

      gLogInfo << "Detected "
               << MaskRCNNConfig::CLASS_NAMES[binfo[roi_id].label] << " in"
               << mOriginalPPMs[p].fileName << " with confidence "
               << binfo[roi_id].prob * 100.f << " and coordinates ("
               << binfo[roi_id].box.x1 << ", " << binfo[roi_id].box.y1 << ", "
               << binfo[roi_id].box.x2 << ", " << binfo[roi_id].box.y2 << ")"
               << std::endl;
    }
    gLogInfo << "The results are stored in current directory: "
             << std::to_string(p) + ".ppm" << std::endl;
