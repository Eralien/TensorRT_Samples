...
//! Command: ./sample_uff_ssd [-h or --help] [-d=/path/to/data/dir or
...
//!
//! \brief The SampleUffSSDParams structure groups the additional parameters
//! required by
//!         the Uff SSD sample.
//!
...
bool SampleUffSSD::verifyOutput(const samplesCommon::BufferManager& buffers) {
  ...
  const int* keepCount = static_cast<const int*>(
  ...
  for (int p = 0; p < batchSize; ++p) {
    ...
    for (int i = 0; i < keepCount[p]; ++i) {
      ...
      gLogInfo << "Detected " << classes[detection].c_str() << " in the image "
               << int(det[0]) << " (" << mPPMs[p].fileName.c_str() << ")"
               << " with confidence " << det[2] * 100.f << " and coordinates ("
               << det[3] * inputW << "," << det[4] * inputH << ")"
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_uff_ssd [-h or --help] [-d or --datadir=<path "
