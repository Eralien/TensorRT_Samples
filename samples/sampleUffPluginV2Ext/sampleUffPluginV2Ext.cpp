...
template <>
void transform<DataType::kHALF, DataType::kFLOAT>(const void* src, void* dst,
                                                  int count) {
  ...
  std::transform(srcPtr, srcPtr + count, dstPtr,
                 [](half_float::half in) { return static_cast<float>(in); });
}
...
template <>
void transform<DataType::kFLOAT, DataType::kHALF>(const void* src, void* dst,
                                                  int count) {
  auto srcPtr = static_cast<const float*>(src);
  auto dstPtr = static_cast<half_float::half*>(dst);
  std::transform(srcPtr, srcPtr + count, dstPtr,
                 [](float in) { return static_cast<half_float::half>(in); });
}
...
void readPGMFile(const std::string& filename,
...
bool verifyOutput(int64_t eltCount, DataType dtype, void* buffer, int num) {
  ...
  int maxIdx = std::distance(outputs.begin(),
...
class SampleUffPluginV2Ext {
 public:
  ...
  bool build() {
    ...
    SampleUniquePtr<IBuilder> builder{
        createInferBuilder(gLogger.getTRTLogger())};
    if (!builder.get()) {
    ...
    if (!parser->parse(mParams.uffFileName.data(), *network,
    ...
    SampleUniquePtr<IBuilderConfig> networkConfig{
    ...
    samplesCommon::enableDLA(builder.get(), networkConfig.get(),
                             gArgs.useDLACore);
  ...
  bool infer() {
    ...
    auto buffersSizes =
        calculateBindingBufferSizes(*mEngine, nbBindings, batchSize);
    ...
    buffers[bindingIdxOutput] = safeCudaMalloc(
        bufferSizesOutput.first * elementSize(bufferSizesOutput.second));
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      for (int num = 0; num < numberRun; num++) {
        buffers[bindingIdxInput] = createMnistCudaBuffer(
            bufferSizesInput.first, bufferSizesInput.second, num);
        ...
        for (int bindingIdx = 0; bindingIdx < nbBindings; ++bindingIdx) {
          ...
          pass &=
              verifyOutput(bufferSizesOutput.first, bufferSizesOutput.second,
                           buffers[bindingIdx], num);
        }
        CHECK(cudaFree(buffers[bindingIdxInput]));
      }
      total /= numberRun;
      gLogInfo << "Average over " << numberRun << " runs is " << total << " ms."
               << std::endl;
    }
...
class UffPoolPluginV2 : public IPluginV2IOExt {
 public:
  ...
  UffPoolPluginV2(const void* data, size_t length) {
    ...
    mMode = mPoolingParams.pType == PoolingType::kMAX
                ? CUDNN_POOLING_MAX
 ...
 public:
  ...
  Dims getOutputDimensions(int index, const Dims* inputs,
    ...
    int height = (inputs[0].d[1] + mPoolingParams.pH * 2 - mPoolingParams.mR) /
                     mPoolingParams.mU +
                 1;
    int width = (inputs[0].d[2] + mPoolingParams.pW * 2 - mPoolingParams.mS) /
                    mPoolingParams.mV +
                1;
  ...
  int initialize() override {
    ...
    CHECK(cudnnSetPooling2dDescriptor(
        mPoolingDesc, mMode, CUDNN_NOT_PROPAGATE_NAN, mPoolingParams.mR,
        mPoolingParams.mS, mPoolingParams.pH, mPoolingParams.pW,
        mPoolingParams.mU, mPoolingParams.mV));
    return 0;
  }
  ...
  int enqueue(int batchSize, const void* const* inputs, void** outputs,
    ...
    if (mDataType == DataType::kINT8) {
      ...
      size_t outCount =
          getC(mOutputDims) * getH(mOutputDims) * getW(mOutputDims);
    ...
    CHECK(cudnnPoolingForward(mCudnn, mPoolingDesc, &kONE, mSrcDescriptor,
  ...
  const char* getPluginType() const override { return "MaxPool"; }
...
class UffPoolPluginV2Creator : public IPluginCreator {
 public:
  const char* getPluginName() const override { return "MaxPool"; }
...
void printHelpInfo() {
  std::cout << "Usage: ./sample_uff_plugin_v2_ext [-h or --help] [-d or "
...
int main(int argc, char** argv) {
  ...
  if (gArgs.dataDirs.empty()) {
    gArgs.dataDirs =
        std::vector<std::string>{"data/samples/mnist/", "data/mnist/"};
  }
