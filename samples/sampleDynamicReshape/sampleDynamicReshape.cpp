...
//! This file contains the implementation of the dynamic reshape MNIST sample.
//! It creates a network
//! using the MNIST ONNX model, and uses a second engine to resize inputs to the
//! shape the model
//! expects.
//! It can be run with the following command:
//! Command: ./sample_dynamic_reshape [-h or --help [-d=/path/to/data/dir or
...
//! \brief The SampleDynamicReshape class implementes the dynamic reshape
//! sample.
//!
//! \details This class builds one engine that resizes a given input to the
//! correct size, and a
...
class SampleDynamicReshape {
 ...
 public:
  ...
  //!
  //! \brief Prepares the model for inference by creating execution contexts and
  //! allocating buffers.
  //!
  void prepare();
 ...
 private:
  void buildPreprocessorEngine(
      const SampleUniquePtr<nvinfer1::IBuilder>& builder);
  void buildPredictionEngine(
      const SampleUniquePtr<nvinfer1::IBuilder>& builder);
  ...
  nvinfer1::Dims mPredictionInputDims;   //!< The dimensions of the input of the
                                         //! MNIST model.
  nvinfer1::Dims mPredictionOutputDims;  //!< The dimensions of the output of
                                         //! the MNIST model.

  // Engines used for inference. The first is used for resizing inputs, the
  // second for prediction.
  SampleUniquePtr<nvinfer1::ICudaEngine> mPreprocessorEngine{nullptr},
      mPredictionEngine{nullptr};

  SampleUniquePtr<nvinfer1::IExecutionContext> mPreprocessorContext{nullptr},
      mPredictionContext{nullptr};

  samplesCommon::ManagedBuffer
      mInput{};  //!< Host and device buffers for the input.
  samplesCommon::DeviceBuffer mPredictionInput{};  //!< Device buffer for the
                                                   //! output of the
  //! preprocessor, i.e. the
...
//!
//! \details This function creates one TensorRT engine for resizing inputs to
//! the correct sizes,
//!          then creates a TensorRT network by parsing the ONNX model and
//!          builds
//!          an engine that will be used to run inference (mPredictionEngine).
//!
void SampleDynamicReshape::build() {
  auto builder =
      makeUnique(nvinfer1::createInferBuilder(gLogger.getTRTLogger()));
...
//!
void SampleDynamicReshape::buildPreprocessorEngine(
    const SampleUniquePtr<nvinfer1::IBuilder>& builder) {
  // Create the preprocessor engine using a network that supports full
  // dimensions (createNetworkV2).
  auto preprocessorNetwork = makeUnique(builder->createNetworkV2(
      1U << static_cast<uint32_t>(
          NetworkDefinitionCreationFlag::kEXPLICIT_BATCH)));

  // Reshape a dynamically shaped input to the size expected by the model, (1,
  // 1, 28, 28).
  auto input = preprocessorNetwork->addInput(
      "input", nvinfer1::DataType::kFLOAT, Dims4{1, 1, -1, -1});
  ...
  // Create an optimization profile so that we can specify a range of input
  // dimensions.
  ...
  mPreprocessorEngine = makeUnique(builder->buildEngineWithConfig(
      *preprocessorNetwork, *preprocessorConfig));
  gLogInfo << "Profile dimensions in preprocessor engine:\n";
  gLogInfo << "    Minimum = "
           << mPreprocessorEngine->getProfileDimensions(
                  0, 0, OptProfileSelector::kMIN)
           << '\n';
  gLogInfo << "    Optimum = "
           << mPreprocessorEngine->getProfileDimensions(
                  0, 0, OptProfileSelector::kOPT)
           << '\n';
  gLogInfo << "    Maximum = "
           << mPreprocessorEngine->getProfileDimensions(
                  0, 0, OptProfileSelector::kMAX)
           << std::endl;
}
...
//!
//! \details This function builds an engine for the MNIST model, and updates
//! mPredictionInputDims and
//! mPredictionOutputDims according to the dimensions specified by the model.
//! The preprocessor reshapes inputs to
//! mPredictionInputDims.
//!
void SampleDynamicReshape::buildPredictionEngine(
    const SampleUniquePtr<nvinfer1::IBuilder>& builder) {
  ...
  // Set softmax axis to 1 since network output has shape [1, 10] in full dims
  // mode
  softmax->setAxes(1 << 1);
  ...
  mPredictionEngine =
      makeUnique(builder->buildEngineWithConfig(*network, *config));
}

//!
//! \brief Prepares the model for inference by creating an execution context and
//! allocating buffers.
//!
//! \details This function sets up the sample for inference. This involves
//! allocating buffers for the inputs and
//! outputs, as well as creating TensorRT execution contexts for both engines.
//! This only needs to be called a single
//! time.
//!
void SampleDynamicReshape::prepare() {
  mPreprocessorContext =
      makeUnique(mPreprocessorEngine->createExecutionContext());
  mPredictionContext = makeUnique(mPredictionEngine->createExecutionContext());
  // Since input dimensions are not known ahead of time, we only allocate the
  // output buffer and preprocessor output
  // buffer.
...
//! It runs inference for using a random image from the MNIST dataset as an
//! input.
//!
bool SampleDynamicReshape::infer() {
  ...
  Dims inputDims =
      loadPGMFile(locateFile(std::to_string(digit) + ".pgm", mParams.dataDirs));
  mInput.deviceBuffer.resize(inputDims);
  CHECK(cudaMemcpy(mInput.deviceBuffer.data(), mInput.hostBuffer.data(),
                   mInput.hostBuffer.nbBytes(), cudaMemcpyHostToDevice));
  ...
  std::vector<void*> preprocessorBindings = {mInput.deviceBuffer.data(),
                                             mPredictionInput.data()};
  // For engines using full dims, we can use executeV2, which does not include a
  // separate batch size parameter.
  ...
  std::vector<void*> predicitonBindings = {mPredictionInput.data(),
  ...
  CHECK(cudaMemcpy(mOutput.hostBuffer.data(), mOutput.deviceBuffer.data(),
                   mOutput.deviceBuffer.nbBytes(), cudaMemcpyDeviceToHost));
  return validateOutput(digit);
}

//!
//! \brief Loads a PGM file into mInput and returns the dimensions of the loaded
//! image.
//!
//! \details This function loads the specified PGM file into the input host
//! buffer.
//!
...
bool SampleDynamicReshape::validateOutput(int digit) {
  ...
  for (const auto& elem : prob) {
    gLogInfo << " Prob " << curIndex << "  " << std::fixed << std::setw(5)
             << std::setprecision(4) << elem << " "
             << "Class " << curIndex << ": "
             << std::string(int(std::floor(elem * 10 + 0.5f)), '*')
             << std::endl;
    ++curIndex;
  }
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_dynamic_reshape [-h or --help] [-d or "
