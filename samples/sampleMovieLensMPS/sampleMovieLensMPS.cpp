...
static const int32_t TOPK_MOVIES{
    1};  // The output of the topK layer for MovieLens sample.
static const int32_t NUM_INDICES{
    100};  // Total numbers of Movies to predict per user.
static const int32_t EMBEDDING_VEC_SIZE{
    32};  // Embedding vector size of each user and item.
...
static const char* TOPK_ITEM_PROB{
    "topk_values"};  // predicted item probability blob name.
static const char* TOPK_ITEM_NAME{
    "topk_items"};  // predicted item probability blob name.
static const char* RATING_INPUT_FILE{
    "movielens_ratings.txt"};  // The default input file with 50 users and
                               // groundtruth data.
static const char* DEFAULT_WEIGHT_FILE{
    "sampleMovieLens.wts2"};  // The weight file produced from README.txt
...
static const std::vector<std::string> directories{"data/samples/movielens/",
                                                  "data/movielens/"};
...
class Semaphore {
 public:
  ...
  void wait() { sem_wait(mSemEngine); }

  void post() { sem_post(mSemEngine); }

  void open() {
    mSemEngine = sem_open(mSemName, O_CREAT | O_EXCL,
...
class SharedMemory {
 public:
  ...
  ~SharedMemory() { shm_unlink(mModelStreamFd); }

  int open_ro() { return open(O_RDONLY, 0666); }

  int open_rw() { return open(O_RDWR | O_CREAT, 0666); }

 private:
  int open(int flag, mode_t mode) {
    int fd = shm_open(mModelStreamFd, flag, mode);
    if (fd <= 0) {
      throw std::runtime_error("Could not create file descriptor: /dev/shm" +
                               std::string(mModelStreamFd));
    }
    return fd;
  }
...
// The OutptutArgs struct holds intermediate/final outputs generated by the
// MovieLens structure per user.
struct OutputArgs {
  int32_t userId;                            // The user Id per batch.
  int32_t expectedPredictedMaxRatingItem;    // The Expected Max Rating Item per
                                             // user (inference ground truth).
  float expectedPredictedMaxRatingItemProb;  // The Expected Max Rating
                                             // Probability. (inference ground
                                             // truth).
  std::vector<int32_t> allItems;             // All inferred items per user.
  std::vector<std::pair<int32_t, float>>
      itemProbPairVec;  // Expected topK items and prob per user.
};                      // struct pargs

struct Args {
  int32_t embeddingVecSize{EMBEDDING_VEC_SIZE};
  int32_t numUsers{NUM_USERS};      // Total number of users. Should be equal to
                                    // ratings file users count.
  ...
  std::string weightFile{
      DEFAULT_WEIGHT_FILE};  // Weight file (.wts2) format Movielens sample.
  ...
  int useDLACore{-1};
  // The below structures are used to compare the predicted values to inference
  // (ground truth)
  std::map<int32_t, std::vector<int32_t>>
      userToItemsMap;  // Lookup for inferred items for each user.
  std::map<int32_t, std::vector<std::pair<int32_t, float>>>
      userToExpectedItemProbMap;  // Lookup for topK items and probs for each
                                  // user.
...
struct Batch {
  Batch(ICudaEngine* engine, void* userInputPtr, void* itemInputPtr,
        const Args& args) {
    mEngine = engine;
    mContext =
        SampleUniquePtr<IExecutionContext>(mEngine->createExecutionContext());
    ...
    CHECK(cudaMallocHost(&mHostMemory[userInputIndex],
                         mMemSizes[userInputIndex]));
    CHECK(cudaMallocHost(&mHostMemory[itemInputIndex],
                         mMemSizes[itemInputIndex]));
    CHECK(cudaMallocHost(&mHostMemory[outputPredictionIndex],
                         mMemSizes[outputPredictionIndex]));
    CHECK(cudaMallocHost(&mHostMemory[outputItemProbIndex],
                         mMemSizes[outputItemProbIndex]));
    CHECK(cudaMallocHost(&mHostMemory[outputItemNameIndex],
                         mMemSizes[outputItemNameIndex]));

    // copy the data to host memory
    for (unsigned int i = 0; i < (mMemSizes[userInputIndex]) / sizeof(float);
         ++i) {
      *(static_cast<uint32_t*>(mHostMemory[userInputIndex]) + i) =
          *((uint32_t*)userInputPtr + i);
    }
    for (unsigned int i = 0; i < (mMemSizes[itemInputIndex]) / sizeof(float);
         ++i) {
      *(static_cast<uint32_t*>(mHostMemory[itemInputIndex]) + i) =
          *((uint32_t*)itemInputPtr + i);
    }

    // allocate GPU memory
    CHECK(
        cudaMalloc(&mDeviceMemory[userInputIndex], mMemSizes[userInputIndex]));
    CHECK(
        cudaMalloc(&mDeviceMemory[itemInputIndex], mMemSizes[itemInputIndex]));
    CHECK(cudaMalloc(&mDeviceMemory[outputPredictionIndex],
                     mMemSizes[outputPredictionIndex]));
    CHECK(cudaMalloc(&mDeviceMemory[outputItemProbIndex],
                     mMemSizes[outputItemProbIndex]));
    CHECK(cudaMalloc(&mDeviceMemory[outputItemNameIndex],
                     mMemSizes[outputItemNameIndex]));
  }

  ~Batch() {
    for (auto p : mHostMemory) CHECK(cudaFreeHost(p));
    for (auto p : mDeviceMemory) CHECK(cudaFree(p));
...
void printHelpInfo() {
  std::cout << "Usage:\n"
            << " ./sample_movielens_mps [-h or --help] [-b NUM_USERS] [-p "
               "NUM_PROCESSES] [--useDLACore=<int>] [--verbose]\n"
            << "-h             Display help information. All single dash "
               "options enable perf mode.\n"
            << "-b             Number of Users i.e. Batch Size (default "
               "numUsers=32).\n"
            << "-p             Number of child processes to launch (default "
               "nbProcesses=1. Using MPS with this option is "
               "strongly recommended).\n"
            << "--useDLACore=N Specify a DLA engine for layers that support "
               "DLA. Value can range from 0 to n-1, where n is "
...
bool parseArgs(Args& args, int argc, char* argv[]) {
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (argStr.compare(0, 13, "--useDLACore=") == 0 &&
               argStr.size() > 13) {
...
void printOutputArgs(OutputArgs& pargs) {
  gLogVerbose << "User Id                            :   " << pargs.userId
              << std::endl;
  gLogVerbose << "Expected Predicted Max Rating Item :   "
              << pargs.expectedPredictedMaxRatingItem << std::endl;
  gLogVerbose << "Expected Predicted Max Rating Prob :   "
              << pargs.expectedPredictedMaxRatingItemProb << std::endl;
  gLogVerbose << "Total TopK Items : " << pargs.itemProbPairVec.size()
              << std::endl;
  for (unsigned i = 0; i < pargs.itemProbPairVec.size(); ++i)
    gLogVerbose << pargs.itemProbPairVec.at(i).first << " : "
                << pargs.itemProbPairVec.at(i).second << std::endl;
}
...
void readInputSample(std::ifstream& file, OutputArgs& pargs, std::string line,
                     const Args& args) {
  // read user name
  char delim = ':';
  auto pos = line.find(delim);
...
bool printInferenceOutput(void* userInputPtr, void* itemInputPtr,
                          void* topKItemNumberPtr, void* topKItemProbPtr,
                          const Args& args) {
  bool pass{true};
  ...
  gLogVerbose
      ...
      << std::endl;
  gLogVerbose
      << "|   User    |   Item     |  Expected Prob  |  Predicted Prob |"
      << std::endl;
  gLogVerbose
      << "|-----------|------------|-----------------|-----------------|"
      << std::endl;
  ...
  for (int i = 0; i < args.numUsers; ++i) {
    ...
    int maxExpectedItem =
        args.userToExpectedItemProbMap.at(userIdx).at(0).first;
    ...
    for (int k = 0; k < args.topKMovies; ++k) {
      ...
      float expectedProb =
          args.userToExpectedItemProbMap.at(userIdx).at(k).second;
      int predictedItem = args.userToItemsMap.at(userIdx).at(predictedIdx);
      gLogVerbose << "|" << std::setw(10) << userIdx << " | " << std::setw(10)
                  << predictedItem << " | " << std::setw(15) << expectedProb
                  << " | " << std::setw(15) << predictedProb << " | "
                  << std::endl;
    }
  }
  ...
  for (int i = 0; i < args.numUsers; ++i) {
    ...
    int maxExpectedItem =
        args.userToExpectedItemProbMap.at(userIdx).at(0).first;
    int maxPredictedItem = args.userToItemsMap.at(userIdx).at(maxPredictedIdx);
    gLogInfo << "| PID : " << std::setw(4) << getpid()
             << " | User :" << std::setw(4) << userIdx
             << "  |  Expected Item :" << std::setw(5) << maxExpectedItem
             << "  |  Predicted Item :" << std::setw(5) << maxPredictedItem
             << " | " << std::endl;
  }
...
void submitWork(Batch& b, const Args& args) {
  ...
  CHECK(cudaMemcpyAsync(
      b.mDeviceMemory[userInputIndex], b.mHostMemory[userInputIndex],
      b.mMemSizes[userInputIndex], cudaMemcpyHostToDevice, b.mStream));
  CHECK(cudaMemcpyAsync(
      b.mDeviceMemory[itemInputIndex], b.mHostMemory[itemInputIndex],
      b.mMemSizes[itemInputIndex], cudaMemcpyHostToDevice, b.mStream));

  b.mContext->enqueue(args.numUsers, b.mDeviceMemory, b.mStream, nullptr);

  // copy output from device to host
  CHECK(cudaMemcpyAsync(b.mHostMemory[outputPredictionIndex],
                        b.mDeviceMemory[outputPredictionIndex],
                        b.mMemSizes[outputPredictionIndex],
                        cudaMemcpyDeviceToHost, b.mStream));
  CHECK(cudaMemcpyAsync(
      b.mHostMemory[outputItemProbIndex], b.mDeviceMemory[outputItemProbIndex],
...
std::shared_ptr<ICudaEngine> loadModelAndCreateEngine(const char* uffFile,
                                                      IUffParser* parser,
                                                      const Args& args) {
  // Create the builder
  ...
  auto dType =
      args.enableFP16 ? nvinfer1::DataType::kHALF : nvinfer1::DataType::kFLOAT;
  ...
  // Reshape output of fully connected layer numOfMovies x 1 x 1 x 1 to
  // numOfMovies x 1 x 1.
  ...
  // Apply TopK layer to retrieve item probabilities and corresponding index
  // number.
  auto topK = network->addTopK(*reshapeLayer->getOutput(0), TopKOperation::kMAX,
                               args.topKMovies, 0x2);
  if (topK == nullptr) {
    throw std::runtime_error("Could not create TopK layer.");
  }

  // Mark outputs for index and probs. Also need to set the item layer type ==
  // kINT32.
  ...
  config->setMaxWorkspaceSize(
      1_GiB);  // The _GiB literal operator is defined in common.h
  if (args.enableFP16) {
  ...
  auto engine = samplesCommon::infer_object(
...
bool doInference(void* modelStreamData, int modelStreamSize, void* userInputPtr,
                 void* itemInputPtr, Args& args) {
  ...
  auto engine = samplesCommon::infer_object(runtime->deserializeCudaEngine(
      modelStreamData, modelStreamSize, nullptr));
...

  {
    ...
    timer.stop();
    gLogInfo << "Done execution in process: " << getpid()
             << " . Duration : " << timer.microseconds() << " microseconds."
             << std::endl;
  }
  ...
  uint32_t* topKItemNumber =
      static_cast<uint32_t*>(b.mHostMemory[outputItemNameIndex]);
  bool pass{printInferenceOutput<uint32_t, float>(
      userInputPtr, itemInputPtr, topKItemNumber, topKItemProb, args)};
...
int mainMovieLensMPS(Args& args, OutputArgs& pargs) {
  ...
  if (isParentProcess) {
    ...
    auto engine =
        loadModelAndCreateEngine(args.uffFile.c_str(), parser.get(), args);
    ...
    void* modelStreamData =
        mmap(NULL, modelStreamSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    std::vector<uint32_t> userInput(args.numUsers * args.numMoviesPerUser *
                                    sizeof(float));
    std::vector<uint32_t> itemInput(args.numUsers * args.numMoviesPerUser *
                                    sizeof(float));
    ...
    // Now wait for parent to construct engine and write the modelstream to the
    // shared buffer.
    sem.wait();
    ...
    if (modelStreamSize <= 0) {
      throw std::runtime_error(
          "Failed to fetch model stream from shared memory buffer.");
    }

    // Retrieve the modelStream and close the file descriptor.
    void* modelStreamData =
        mmap(NULL, modelStreamSize, PROT_READ, MAP_SHARED, fd, 0);
    close(fd);

    // All child processes will do inference and then exit.
    bool pass = doInference(modelStreamData, modelStreamSize, userInput.data(),
                            itemInput.data(), args);
    if (!pass)
      args.failCount++;
  ...
  // Then time them.
  {
    ...
    gLogInfo << "Number of processes executed : " << args.nbProcesses
             << ". Total MPS Run Duration : " << timer.milliseconds()
