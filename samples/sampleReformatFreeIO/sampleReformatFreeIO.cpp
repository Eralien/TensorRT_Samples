...
//! \brief This file contains the implementation of the reformat free I/O
//! sample.
//!
//! It builds a TensorRT engine by constructing a conv layer. It uses the engine
//! to run
...
//!
//! \brief  The SampleReformatFreeIO class implements the reformat free I/O
//! sample
//!
...
class SampleReformatFreeIO {
 ...
 private:
  //!
  //! \brief uses a Caffe parser to create the single layer Network and marks
  //! the
  //!        output layers
  //!
...
//! engine
//!
//! \details This function creates the single layer network by manual insertion
//! and builds
//!          the engine
//!
...
bool SampleReformatFreeIO::build(int dataWidth) {
  ...
  network->getInput(0)->setAllowedFormats(
      static_cast<TensorFormats>(1 << static_cast<int>(mTensorFormat)));
...
void SampleReformatFreeIO::constructNetwork(
  ...
  mMeanBlob = SampleUniquePtr<nvcaffeparser1::IBinaryProtoBlob>(
      parser->parseBinaryProto(mParams.meanFileName.c_str()));
  nvinfer1::Weights meanWeights{nvinfer1::DataType::kFLOAT,
                                mMeanBlob->getData(),
                                inputDims.d[1] * inputDims.d[2]};
  // For this sample, a large range based on the mean data is chosen and applied
  // to the entire network.
  // The preferred method is use scales computed based on a representative data
  // set
  // and apply each one individually based on the tensor. The range here is
  // large enough for the
  // network, but is chosen for example purposes only.
  float maxMean =
      samplesCommon::getMaxValue(static_cast<const float*>(meanWeights.values),
                                 samplesCommon::volume(inputDims));
  ...
  samplesCommon::setAllTensorScales(network.get(), maxMean / maxMean * 128,
                                    128);
}
...
bool SampleReformatFreeIO::infer(SampleBuffer& inputBuf,
  ...
  CHECK(cudaMemcpy(devInput.get(), inputBuf.buffer, inputBuf.getBufferSize(),
  ...
  CHECK(cudaMemcpy(outputBuf.buffer, devOutput.get(), outputBuf.getBufferSize(),
...
bool SampleReformatFreeIO::readDigits(SampleBuffer& buffer,
  ...
  readPGMFile(
      locateFile(std::to_string(groundTruthDigit) + ".pgm", mParams.dataDirs),
...
int calcIndex(SampleBuffer& buffer, int c, int h, int w) {
  ...
  if (!buffer.desc.channelPivot) {
    index = c / buffer.desc.dims[3] * buffer.desc.dims[1] *
                buffer.desc.dims[2] * buffer.desc.dims[3] +
            h * buffer.desc.dims[2] * buffer.desc.dims[3] +
            w * buffer.desc.dims[3] + c % buffer.desc.dims[3];
  } else {
...
//!
//! \brief Reformats the buffer. Src and dst buffers should be of same datatype
//! and dims.
//!
template <typename T>
...
//!
samplesCommon::CaffeSampleParams initializeSampleParams(
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_reformat_free_io [-h or --help] [-d or "
...
int process(SampleReformatFreeIO& sample, const Logger::TestAtom& sampleTest,
  ...
  gLogInfo
      << "Building and running a GPU inference engine for reformat free I/O"
      << std::endl;
  ...
  SampleBuffer linearOutputBuf(sample.mOutputDims, sizeof(T),
...
int runFP32Reference(SampleReformatFreeIO& sample,
                     const Logger::TestAtom& sampleTest,
                     SampleBuffer& goldenInput, SampleBuffer& goldenOutput) {
  gLogInfo
      << "Building and running a FP32 GPU inference to get golden input/output"
      << std::endl;
  ...
  goldenInput =
      SampleBuffer(sample.mInputDims, sizeof(float), TensorFormat::kLINEAR);
  goldenOutput =
      SampleBuffer(sample.mOutputDims, sizeof(float), TensorFormat::kLINEAR);
...
int main(int argc, char** argv) {
  ...
  gLogInfo << "The test chooses MNIST as the network and recognizes a randomly "
              "generated digit"
           << std::endl;
  gLogInfo << "Firstly it runs the FP32 as the golden data, then INT8/FP16 "
              "with different formats will be tested"
           << std::endl
           << std::endl;
  ...
  for (auto elem : vecINT8TensorFmt) {
    gLogInfo << "Testing datatype INT8 with format " << elem.second
             << std::endl;
    sample.mTensorFormat = elem.first;
    SampleBuffer inputBuf, outputBuf;

    process<int8_t>(sample, sampleTest, inputBuf, outputBuf, goldenInput,
                    goldenOutput);
  }

  // Test FP16 formats
  for (auto elem : vecFP16TensorFmt) {
    gLogInfo << "Testing datatype FP16 with format " << elem.second
             << std::endl;
    sample.mTensorFormat = elem.first;
    SampleBuffer inputBuf, outputBuf;

    process<half_float::half>(sample, sampleTest, inputBuf, outputBuf,
                              goldenInput, goldenOutput);
  }
