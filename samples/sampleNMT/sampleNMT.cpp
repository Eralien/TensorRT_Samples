...
nmtSample::Vocabulary::ptr gOutputVocabulary =
    std::make_shared<nmtSample::Vocabulary>();

std::string locateNMTFile(const std::string& fpathSuffix) {
  std::vector<std::string> dirs{std::string(gDataDirectory) + "/",
                                "data/nmt/deen/"};
...
nmtSample::DataReader::ptr getDataReader() {
  std::shared_ptr<std::istream> textInput(
      new std::ifstream(locateNMTFile(gInputTextFileName)));
  std::shared_ptr<std::istream> vocabInput(
      new std::ifstream(locateNMTFile(gInputVocabularyFileName)));
  ...
  *vocabInput >> *vocabulary;
  ...
  if (gMaxInferenceSamples >= 0)
    return std::make_shared<nmtSample::LimitedSamplesDataReader>(
        gMaxInferenceSamples, reader);
  else
    return reader;
}

template <typename Component>
std::shared_ptr<Component> buildNMTComponentFromWeightsFile(
...
nmtSample::Embedder::ptr getInputEmbedder() {
  return buildNMTComponentFromWeightsFile<nmtSample::SLPEmbedder>(
      gEncEmbedFileName);
}
...
nmtSample::Encoder::ptr getEncoder() {
  return buildNMTComponentFromWeightsFile<nmtSample::LSTMEncoder>(
      gEncRnnFileName);
}

nmtSample::Alignment::ptr getAlignment() {
  return buildNMTComponentFromWeightsFile<nmtSample::MultiplicativeAlignment>(
      gDecMemFileName);
}
...
nmtSample::Decoder::ptr getDecoder() {
  return buildNMTComponentFromWeightsFile<nmtSample::LSTMDecoder>(
      gDecRnnFileName);
}

nmtSample::Attention::ptr getAttention() {
  return buildNMTComponentFromWeightsFile<nmtSample::SLPAttention>(
      gDecAttFileName);
}

nmtSample::Projection::ptr getProjection() {
  return buildNMTComponentFromWeightsFile<nmtSample::SLPProjection>(
      gDecProjFileName);
}
...
nmtSample::BeamSearchPolicy::ptr getSearchPolicy(
    int endSequenceId, nmtSample::LikelihoodCombinationOperator::ptr
                           likelihoodCombinationOperator) {
  return std::make_shared<nmtSample::BeamSearchPolicy>(
      endSequenceId, likelihoodCombinationOperator, gBeamWidth);
}

nmtSample::DataWriter::ptr getDataWriter() {
  if (gDataWriterStr == "bleu") {
    std::shared_ptr<std::istream> textInput(
        new std::ifstream(locateNMTFile(gReferenceOutputTextFileName)));
    assert(textInput->good());
    return std::make_shared<nmtSample::BLEUScoreWriter>(textInput,
                                                        gOutputVocabulary);
  } else if (gDataWriterStr == "text") {
    std::remove(gOutputTextFileName.data());
    std::shared_ptr<std::ostream> textOutput(
        new std::ofstream(gOutputTextFileName));
    assert(textOutput->good() &&
           "Please contact system administrator if you have no permission to "
           "write the file "
           "translation_output.txt");
    return std::make_shared<nmtSample::TextWriter>(textOutput,
                                                   gOutputVocabulary);
  } else if (gDataWriterStr == "benchmark") {
    return std::make_shared<nmtSample::BenchmarkWriter>();
  } else {
    gLogError << "Invalid data writer specified: " << gDataWriterStr
              << std::endl;
    assert(0);
...
bool parseBool(const char* arg, const char* longName, bool& value,
               char shortName = 0) {
  bool match = false;
...
void printUsage() {
  ...
  printf(
      "  --data_writer=bleu/text/benchmark    Type of the output the app "
      "generates (default = %s)\n",
      gDataWriterStr.c_str());
  printf(
      "  --output_file=<path_to_file>         Path to the output file when "
      "data_writer=text (default = %s)\n",
  ...
  printf("  --beam=<N>                           Beam width (default = %d)\n",
         gBeamWidth);
  printf(
      "  --max_input_sequence_length=<N>      Maximum length for input "
      "sequences (default = %d)\n",
      gMaxInputSequenceLength);
  printf(
      "  --max_output_sequence_length=<N>     Maximum length for output "
      "sequences (default = %d), negative value "
      "indicates no limit\n",
      gMaxOutputSequenceLength);
  printf(
      "  --max_inference_samples=<N>          Maximum sample count to run "
      "inference for, negative values indicates "
      "no limit is set (default = %d)\n",
      gMaxInferenceSamples);
  printf(
      "  --verbose                            Output verbose-level messages by "
      "TensorRT\n");
  printf(
      "  --max_workspace_size=<N>             Maximum workspace size (default "
      "= %d)\n",
      gMaxWorkspaceSize);
  printf(
      "  --data_dir=<path_to_data_directory>  Path to the directory where data "
      "and weights are located (default = "
      "%s)\n",
      gDataDirectory.c_str());
  printf(
      "  --profile                            Profile TensorRT execution layer "
      "by layer. Use benchmark data_writer "
      "when profiling on, disregard benchmark results\n");
  printf(
      "  --aggregate_profile                  Merge profiles from multiple "
      "TensorRT engines\n");
  printf("  --fp16                               Switch on fp16 math\n");
  printf("  --int8                               Switch on int8 math\n");
  printf(
      "  --useDLACore=N                       Specify a DLA engine for layers "
      "that support DLA. Value can range from "
      "0 to n-1, where n is the number of DLA engines on the platform.\n");
  printf(
      "  --padMultiple=N                      Specify multiple to pad out "
      "matrix dimensions to test performance\n");
}
...
bool parseNMTArgs(samplesCommon::Args& args, int argc, char* argv[]) {
  ...
  for (int j = 1; j < argc; j++) {
    ...
    if (parseInt(argv[j], "max_input_sequence_length", gMaxInputSequenceLength))
      continue;
    if (parseInt(argv[j], "max_output_sequence_length",
                 gMaxOutputSequenceLength))
      continue;
...
nvinfer1::ICudaEngine* getEncoderEngine(nmtSample::Embedder::ptr inputEmbedder,
                                        nmtSample::Encoder::ptr encoder,
                                        nmtSample::Alignment::ptr alignment) {
  nvinfer1::IBuilder* encoderBuilder =
      nvinfer1::createInferBuilder(gLogger.getTRTLogger());
  assert(encoderBuilder != nullptr);
  nvinfer1::IBuilderConfig* encoderConfig =
      encoderBuilder->createBuilderConfig();
  ...
  nvinfer1::INetworkDefinition* encoderNetwork =
      encoderBuilder->createNetwork();

  // Define inputs for the encoder
  nvinfer1::Dims inputDims{
      1, {gMaxInputSequenceLength}, {nvinfer1::DimensionType::kSEQUENCE}};
  auto inputEncoderDataTensor = encoderNetwork->addInput(
      "input_encoder_data", nvinfer1::DataType::kINT32, inputDims);
  ...
  auto actualInputSequenceLengthsTensor = encoderNetwork->addInput(
      "actual_input_sequence_lengths", nvinfer1::DataType::kINT32,
      inputSequenceLengthsDims);
  assert(actualInputSequenceLengthsTensor != nullptr);
  nvinfer1::Dims inputSequenceLengthsWithUnitIndexDims{
      1, {1}, {nvinfer1::DimensionType::kINDEX}};
  auto actualInputSequenceLengthsWithUnitIndexTensor = encoderNetwork->addInput(
      "actual_input_sequence_lengths_with_index_dim",
      nvinfer1::DataType::kINT32, inputSequenceLengthsWithUnitIndexDims);
  ...
  for (int i = 0; i < static_cast<int>(stateSizes.size()); ++i) {
    ...
    encoderInputStatesTensors[i] = encoderNetwork->addInput(
        ss.str().c_str(),
        gFp16 ? nvinfer1::DataType::kHALF : nvinfer1::DataType::kFLOAT,
        stateSizes[i]);
  ...
  if (gInitializeDecoderFromEncoderHiddenStates) {
    nvinfer1::Dims inputDims{
        1, {gBeamWidth}, {nvinfer1::DimensionType::kINDEX}};
    initializeDecoderIndicesTensor = encoderNetwork->addInput(
        "initialize_decoder_indices", nvinfer1::DataType::kINT32, inputDims);
    assert(initializeDecoderIndicesTensor != nullptr);
  }

  nvinfer1::ITensor* inputEncoderEmbeddedTensor;
  inputEmbedder->addToModel(encoderNetwork, inputEncoderDataTensor,
  ...
  encoder->addToModel(
      encoderNetwork, gMaxInputSequenceLength, inputEncoderEmbeddedTensor,
      actualInputSequenceLengthsTensor, &encoderInputStatesTensors[0],
      &memoryStatesTensor,
      gInitializeDecoderFromEncoderHiddenStates ? &encoderOutputStatesTensors[0]
                                                : nullptr);
  memoryStatesTensor->setName("memory_states");
  encoderNetwork->markOutput(*memoryStatesTensor);
  memoryStatesTensor->setType(gFp16 ? nvinfer1::DataType::kHALF
                                    : nvinfer1::DataType::kFLOAT);

  if (alignment->getAttentionKeySize() > 0) {
    nvinfer1::ITensor* attentionKeysTensor;
    alignment->addAttentionKeys(encoderNetwork, memoryStatesTensor,
                                &attentionKeysTensor);
    attentionKeysTensor->setName("attention_keys");
    encoderNetwork->markOutput(*attentionKeysTensor);
    attentionKeysTensor->setType(gFp16 ? nvinfer1::DataType::kHALF
                                       : nvinfer1::DataType::kFLOAT);
  }
  ...
  // Replicate sequence lengths for the decoder
  {
    auto gatherLayer = encoderNetwork->addGather(
        *actualInputSequenceLengthsWithUnitIndexTensor,
        *initializeDecoderIndicesTensor, 0);
    ...
    actualInputSequenceLengthsReplicatedTensor->setName(
        "actual_input_sequence_lengths_replicated");
    encoderNetwork->markOutput(*actualInputSequenceLengthsReplicatedTensor);
    actualInputSequenceLengthsReplicatedTensor->setType(
  ...
  if (gInitializeDecoderFromEncoderHiddenStates) {
    for (int i = 0; i < static_cast<int>(stateSizes.size()); ++i) {
      ...
      {
        auto shuffleLayer =
            encoderNetwork->addShuffle(*encoderOutputStatesTensors[i]);
      ...
      auto gatherLayer =
          encoderNetwork->addGather(*encoderOutputStatesTensorWithUnitIndex,
                                    *initializeDecoderIndicesTensor, 0);
      assert(gatherLayer != nullptr);
      {
        std::stringstream ss;
      ...
      inputDecoderHiddenStatesTensor->setType(
          gFp16 ? nvinfer1::DataType::kHALF : nvinfer1::DataType::kFLOAT);
    }
  }
  ...
  auto res =
      encoderBuilder->buildEngineWithConfig(*encoderNetwork, *encoderConfig);
...
nvinfer1::ICudaEngine* getGeneratorEngine(
    nmtSample::Embedder::ptr outputEmbedder, nmtSample::Decoder::ptr decoder,
    nmtSample::Alignment::ptr alignment, nmtSample::Context::ptr context,
    nmtSample::Attention::ptr attention, nmtSample::Projection::ptr projection,
    nmtSample::Likelihood::ptr likelihood) {
  nvinfer1::IBuilder* generatorBuilder =
      nvinfer1::createInferBuilder(gLogger.getTRTLogger());
  assert(generatorBuilder != nullptr);
  nvinfer1::IBuilderConfig* generatorConfig =
      generatorBuilder->createBuilderConfig();
  ...
  nvinfer1::INetworkDefinition* generatorNetwork =
      generatorBuilder->createNetwork();
  ...
  nvinfer1::Dims inputDecoderDataDims{
      1, {gBeamWidth}, {nvinfer1::DimensionType::kINDEX}};
  auto inputDecoderDataTensor = generatorNetwork->addInput(
      "input_decoder_data", nvinfer1::DataType::kINT32, inputDecoderDataDims);
  ...
  auto actualInputSequenceLengthsReplicatedTensor = generatorNetwork->addInput(
      "actual_input_sequence_lengths_replicated", nvinfer1::DataType::kINT32,
      inputSequenceLengthsTeplicatedDims);
  assert(actualInputSequenceLengthsReplicatedTensor != nullptr);
  nvinfer1::Dims memoryStatesDims{
      2,
      {gMaxInputSequenceLength, alignment->getSourceStatesSize()},
  ...
  if (alignment->getAttentionKeySize() > 0) {
    nvinfer1::Dims attentionKeysDims{
        2,
        {gMaxInputSequenceLength, alignment->getAttentionKeySize()},
  ...
  if (gFeedAttentionToInput) {
    nvinfer1::Dims inputAttentionDims{
        2,
        {gBeamWidth, attention->getAttentionSize()},
        {nvinfer1::DimensionType::kINDEX, nvinfer1::DimensionType::kCHANNEL}};
  ...
  nvinfer1::Dims inputLikelihoodsDims{
      ...
      {gBeamWidth, 1},
      {nvinfer1::DimensionType::kINDEX, nvinfer1::DimensionType::kCHANNEL}};
  auto inputLikelihoodsTensor = generatorNetwork->addInput(
      "input_likelihoods", nvinfer1::DataType::kFLOAT, inputLikelihoodsDims);
  assert(inputLikelihoodsTensor != nullptr);
  nvinfer1::Dims inputLikelihoodsReplicateIndicesDims{
      1, {gBeamWidth}, {nvinfer1::DimensionType::kCHANNEL}};
  auto inputLikelihoodsReplicateIndicesTensor = generatorNetwork->addInput(
      "replicate_likelihoods_indices", nvinfer1::DataType::kINT32,
      inputLikelihoodsReplicateIndicesDims);
  assert(inputLikelihoodsReplicateIndicesTensor != nullptr);

  // Add output embedder
  nvinfer1::ITensor* inputDecoderEmbeddedTensor;
  outputEmbedder->addToModel(generatorNetwork, inputDecoderDataTensor,
                             &inputDecoderEmbeddedTensor);
  assert(inputDecoderEmbeddedTensor != nullptr);

  // Add concatination of previous attention vector and embedded input for the
  // decoder
  nvinfer1::ITensor* inputDecoderEmbeddedConcatinatedWithAttentionTensor{
      nullptr};
  if (gFeedAttentionToInput) {
    nvinfer1::ITensor* inputTensors[] = {inputDecoderEmbeddedTensor,
                                         inputAttentionTensor};
    ...
    inputDecoderEmbeddedConcatinatedWithAttentionTensor =
        concatLayer->getOutput(0);
  ...
  decoder->addToModel(generatorNetwork,
                      gFeedAttentionToInput
                          ? inputDecoderEmbeddedConcatinatedWithAttentionTensor
                          : inputDecoderEmbeddedTensor,
                      &decoderInputStatesTensors[0], &outputDecoderDataTensor,
                      &decoderOutputStatesTensors[0]);
  for (int i = 0; i < static_cast<int>(stateSizes.size()); ++i) {
    std::stringstream ss;
    ...
    decoderOutputStatesTensors[i]->setType(gFp16 ? nvinfer1::DataType::kHALF
                                                 : nvinfer1::DataType::kFLOAT);
  }

  // Add alignment scores
  nvinfer1::ITensor* alignmentScoresTensor;
  alignment->addToModel(generatorNetwork, (alignment->getAttentionKeySize() > 0)
                                              ? attentionKeysTensor
                                              : memoryStatesTensor,
                        outputDecoderDataTensor, &alignmentScoresTensor);

  // Add context
  nvinfer1::ITensor* contextTensor;
  context->addToModel(
      generatorNetwork, actualInputSequenceLengthsReplicatedTensor,
      memoryStatesTensor, alignmentScoresTensor, &contextTensor);

  // Add attention
  nvinfer1::ITensor* attentionTensor;
  attention->addToModel(generatorNetwork, outputDecoderDataTensor,
                        contextTensor, &attentionTensor);
  if (gFeedAttentionToInput) {
    attentionTensor->setName("output_attention");
    generatorNetwork->markOutput(*attentionTensor);
    attentionTensor->setType(gFp16 ? nvinfer1::DataType::kHALF
                                   : nvinfer1::DataType::kFLOAT);
  }
  ...
  auto gatherLayer = generatorNetwork->addGather(
      *inputLikelihoodsTensor, *inputLikelihoodsReplicateIndicesTensor, 1);
  assert(gatherLayer != nullptr);
  ...
  likelihood->addToModel(
      generatorNetwork, gBeamWidth, logitsTensor,
      inputLikelihoodsReplicatedTensor, &outputCombinedLikelihoodsTensor,
      &outputRayOptionIndicesTensor, &outputVocabularyIndicesTensor);
  ...
  auto res = generatorBuilder->buildEngineWithConfig(*generatorNetwork,
                                                     *generatorConfig);
...
nvinfer1::ICudaEngine* getGeneratorShuffleEngine(
  ...
  nvinfer1::IBuilder* shuffleBuilder =
      nvinfer1::createInferBuilder(gLogger.getTRTLogger());
  assert(shuffleBuilder != nullptr);
  nvinfer1::IBuilderConfig* shuffleConfig =
      shuffleBuilder->createBuilderConfig();
  ...
  nvinfer1::INetworkDefinition* shuffleNetwork =
      shuffleBuilder->createNetwork();

  nvinfer1::Dims sourceRayIndicesDims{
      1, {gBeamWidth}, {nvinfer1::DimensionType::kINDEX}};
  auto sourceRayIndicesTensor = shuffleNetwork->addInput(
      "source_ray_indices", nvinfer1::DataType::kINT32, sourceRayIndicesDims);
  assert(sourceRayIndicesTensor != nullptr);

  std::vector<nvinfer1::ITensor*> previousOutputDecoderStatesTensors(
      decoderStateSizes.size());
  ...
  if (gFeedAttentionToInput) {
    nvinfer1::Dims previousOutputAttentionDims{
        2,
        {gBeamWidth, attentionSize},
        {nvinfer1::DimensionType::kINDEX, nvinfer1::DimensionType::kCHANNEL}};
    previousOutputAttentionTensor = shuffleNetwork->addInput(
        "previous_output_attention",
        gFp16 ? nvinfer1::DataType::kHALF : nvinfer1::DataType::kFLOAT,
        previousOutputAttentionDims);
    assert(previousOutputAttentionTensor != nullptr);
  }

  for (int i = 0; i < static_cast<int>(decoderStateSizes.size()); ++i) {
    auto gatherLayer = shuffleNetwork->addGather(
        *previousOutputDecoderStatesTensors[i], *sourceRayIndicesTensor, 0);
    assert(gatherLayer != nullptr);
    {
      std::stringstream ss;
    ...
    inputDecoderHiddenStatesTensor->setType(gFp16 ? nvinfer1::DataType::kHALF
                                                  : nvinfer1::DataType::kFLOAT);
  }

  if (gFeedAttentionToInput) {
    auto gatherLayer = shuffleNetwork->addGather(*previousOutputAttentionTensor,
                                                 *sourceRayIndicesTensor, 0);
    assert(gatherLayer != nullptr);
    ...
    inputAttentionTensor->setType(gFp16 ? nvinfer1::DataType::kHALF
                                        : nvinfer1::DataType::kFLOAT);
  }
  ...
  auto res =
      shuffleBuilder->buildEngineWithConfig(*shuffleNetwork, *shuffleConfig);
...
//! \brief assign device pointers to the correct location in the bindings
//! vector.
//!
...
void processBindings(std::vector<void*>& bindings,
...
int main(int argc, char** argv) {
  ...
  auto searchPolicy =
      getSearchPolicy(outputSequenceProperties->getEndSequenceId(),
                      likelihood->getLikelihoodCombinationOperator());
  ...
  if (gInitializeDecoderFromEncoderHiddenStates) {
    ...
    for (int i = 0; i < static_cast<int>(stateSizes.size()); ++i)
      assert(nmtSample::getVolume(stateSizes[i]) ==
             nmtSample::getVolume(encoderStateSizes[i]));
  }
  assert(projection->getOutputSize() ==
         outputEmbedder->getInputDimensionSize());

  auto inputOriginalHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<int>>(
          gMaxBatchSize * gMaxInputSequenceLength);
  auto inputHostBuffer = std::make_shared<nmtSample::PinnedHostBuffer<int>>(
      gMaxBatchSize * gMaxInputSequenceLength);
  auto inputOriginalSequenceLengthsHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<int>>(gMaxBatchSize);
  auto inputSequenceLengthsHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<int>>(gMaxBatchSize);
  auto maxOutputSequenceLengthsHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<int>>(gMaxBatchSize);
  auto outputSequenceLengthsHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<int>>(gMaxBatchSize);
  auto outputCombinedLikelihoodHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<float>>(gMaxBatchSize *
                                                           gBeamWidth);
  ...
  auto sourceRayIndicesHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<int>>(gMaxBatchSize *
                                                         gBeamWidth);
  auto sourceLikelihoodsHostBuffer =
      std::make_shared<nmtSample::PinnedHostBuffer<float>>(gMaxBatchSize *
                                                           gBeamWidth);

  // Allocated buffers on GPU to be used as inputs and outputs for TenorRT
  auto inputEncoderDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize *
                                                     gMaxInputSequenceLength);
  auto inputSequenceLengthsDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize);
  auto inputSequenceLengthsReplicatedDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize *
                                                     gBeamWidth);
  auto memoryStatesDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(
          gMaxBatchSize * gMaxInputSequenceLength *
          encoder->getMemoryStatesSize());
  auto attentionKeysDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(
          (alignment->getAttentionKeySize() > 0)
              ? gMaxBatchSize * gMaxInputSequenceLength *
                    alignment->getAttentionKeySize()
              : 0);
  std::vector<nmtSample::DeviceBuffer<float>::ptr>
      encoderStatesLastTimestepDeviceBuffers;
  for (auto stateSize : stateSizes)
    encoderStatesLastTimestepDeviceBuffers.push_back(
        std::make_shared<nmtSample::DeviceBuffer<float>>(
            gMaxBatchSize * nmtSample::getVolume(stateSize)));
  std::vector<nmtSample::DeviceBuffer<float>::ptr>
      inputDecoderStatesDeviceBuffers;
  for (auto stateSize : stateSizes)
    inputDecoderStatesDeviceBuffers.push_back(
        std::make_shared<nmtSample::DeviceBuffer<float>>(
            gMaxBatchSize * gBeamWidth * nmtSample::getVolume(stateSize)));
  std::vector<nmtSample::DeviceBuffer<float>::ptr>
      outputDecoderStatesDeviceBuffers;
  for (auto stateSize : stateSizes)
    outputDecoderStatesDeviceBuffers.push_back(
        std::make_shared<nmtSample::DeviceBuffer<float>>(
            gMaxBatchSize * gBeamWidth * nmtSample::getVolume(stateSize)));
  auto inputAttentionDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(
          gFeedAttentionToInput
              ? gMaxBatchSize * gBeamWidth * attention->getAttentionSize()
              : 0);
  auto outputAttentionDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(
          gFeedAttentionToInput
              ? gMaxBatchSize * gBeamWidth * attention->getAttentionSize()
              : 0);
  auto outputCombinedLikelihoodDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(gMaxBatchSize *
                                                       gBeamWidth);
  auto outputRayOptionIndicesDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize *
                                                     gBeamWidth);
  auto sourceRayIndicesDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize *
                                                     gBeamWidth);
  auto inputDecoderDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize *
                                                     gBeamWidth);
  auto inputLikelihoodsDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(gMaxBatchSize *
                                                       gBeamWidth);

  std::vector<nmtSample::DeviceBuffer<float>::ptr>
      zeroInputEncoderStatesDeviceBuffers;
  for (auto stateSize : stateSizes) {
    auto buf = std::make_shared<nmtSample::DeviceBuffer<float>>(
        gMaxBatchSize * nmtSample::getVolume(stateSize));
    CUDA_CHECK(cudaMemsetAsync(
        *buf, 0,
        gMaxBatchSize * nmtSample::getVolume(stateSize) * sizeof(float),
        stream));
    zeroInputEncoderStatesDeviceBuffers.push_back(buf);
  }

  std::vector<nmtSample::DeviceBuffer<float>::ptr>
      zeroInputDecoderStatesDeviceBuffers;
  for (auto stateSize : stateSizes) {
    auto buf = std::make_shared<nmtSample::DeviceBuffer<float>>(
        gMaxBatchSize * gBeamWidth * nmtSample::getVolume(stateSize));
    CUDA_CHECK(cudaMemsetAsync(*buf, 0, gMaxBatchSize * gBeamWidth *
                                            nmtSample::getVolume(stateSize) *
                                            sizeof(float),
                               stream));
    zeroInputDecoderStatesDeviceBuffers.push_back(buf);
  }

  auto zeroInputAttentionDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(
          gFeedAttentionToInput
              ? gMaxBatchSize * gBeamWidth * attention->getAttentionSize()
              : 0);
  if (gFeedAttentionToInput) {
    CUDA_CHECK(cudaMemsetAsync(*zeroInputAttentionDeviceBuffer, 0,
                               gMaxBatchSize * gBeamWidth *
                                   attention->getAttentionSize() *
                                   sizeof(float),
                               stream));
  }
                                                     ...
                                                     gBeamWidth);
  {
    auto startSeqInputDecoderHostBuffer =
        std::make_shared<nmtSample::PinnedHostBuffer<int>>(gMaxBatchSize *
                                                           gBeamWidth);
    std::fill_n((int*)*startSeqInputDecoderHostBuffer,
                gMaxBatchSize * gBeamWidth,
                outputSequenceProperties->getStartSequenceId());
    CUDA_CHECK(cudaMemcpyAsync(*startSeqInputDecoderDeviceBuffer,
                               *startSeqInputDecoderHostBuffer,
                               gMaxBatchSize * gBeamWidth * sizeof(int),
  ...
  auto zeroInitializeDecoderIndicesDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize *
                                                     gBeamWidth);
  CUDA_CHECK(cudaMemsetAsync(*zeroInitializeDecoderIndicesDeviceBuffer, 0,
                             gMaxBatchSize * gBeamWidth * sizeof(int), stream));
  auto initialInputLikelihoodsDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<float>>(gMaxBatchSize *
                                                       gBeamWidth);
  {
    auto likelihoodCombinationOperator =
        likelihood->getLikelihoodCombinationOperator();
    auto initialInputLikelihoodsHostBuffer =
        std::make_shared<nmtSample::PinnedHostBuffer<float>>(gMaxBatchSize *
                                                             gBeamWidth);
    for (int sampleId = 0; sampleId < gMaxBatchSize; ++sampleId) {
      (*initialInputLikelihoodsHostBuffer)[sampleId * gBeamWidth] =
          likelihoodCombinationOperator->init();
    ...
    CUDA_CHECK(cudaMemcpyAsync(*initialInputLikelihoodsDeviceBuffer,
                               *initialInputLikelihoodsHostBuffer,
                               gMaxBatchSize * gBeamWidth * sizeof(float),
                               cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
  }
  auto zeroReplicateLikelihoodsIndicesDeviceBuffer =
      std::make_shared<nmtSample::DeviceBuffer<int>>(gMaxBatchSize *
                                                     gBeamWidth);
  CUDA_CHECK(cudaMemsetAsync(*zeroReplicateLikelihoodsIndicesDeviceBuffer, 0,
                             gMaxBatchSize * gBeamWidth * sizeof(int), stream));

  // Create TensorRT engines
  nvinfer1::ICudaEngine* encoderEngine =
      getEncoderEngine(inputEmbedder, encoder, alignment);
  nvinfer1::ICudaEngine* generatorEngine =
      getGeneratorEngine(outputEmbedder, decoder, alignment, context, attention,
                         projection, likelihood);
  nvinfer1::ICudaEngine* generatorShuffleEngine = getGeneratorShuffleEngine(
      decoder->getStateSizes(), attention->getAttentionSize());
  ...
  encBindingMap["actual_input_sequence_lengths"] =
      *inputSequenceLengthsDeviceBuffer;
  encBindingMap["actual_input_sequence_lengths_with_index_dim"] =
      *inputSequenceLengthsDeviceBuffer;
  encBindingMap["actual_input_sequence_lengths_replicated"] =
      *inputSequenceLengthsReplicatedDeviceBuffer;
  encBindingMap["initialize_decoder_indices"] =
      *zeroInitializeDecoderIndicesDeviceBuffer;
  for (int i = 0; i < static_cast<int>(stateSizes.size()); ++i) {
    std::stringstream ss;
    ss << "input_encoder_states_" << i;
  ...
  genBindingMap["actual_input_sequence_lengths_replicated"] =
      *inputSequenceLengthsReplicatedDeviceBuffer;
  ...
  genBindingMap["output_combined_likelihoods"] =
      *outputCombinedLikelihoodDeviceBuffer;
  genBindingMap["output_vocabulary_indices"] = *inputDecoderDeviceBuffer;
  genBindingMap["output_ray_option_indices"] =
      *outputRayOptionIndicesDeviceBuffer;
  if (gFeedAttentionToInput) {
  ...
  genBindingMap["replicate_likelihoods_indices"] =
      *zeroReplicateLikelihoodsIndicesDeviceBuffer;
  ...
  genBindingFirstStepMap["input_decoder_data"] =
      *startSeqInputDecoderDeviceBuffer;
  if (!gInitializeDecoderFromEncoderHiddenStates) {
    for (int i = 0; i < static_cast<int>(stateSizes.size()); ++i) {
      std::stringstream ss;
      ss << "input_decoder_states_" << i;
      genBindingFirstStepMap[ss.str()] =
          *zeroInputDecoderStatesDeviceBuffers[i];
    }
  }
  if (gFeedAttentionToInput) {
    genBindingFirstStepMap["input_attention"] = *zeroInputAttentionDeviceBuffer;
  }
  genBindingFirstStepMap["input_likelihoods"] =
      *initialInputLikelihoodsDeviceBuffer;
  processBindings(generatorBindingsFirstStep, genBindingFirstStepMap,
                  generatorEngine);

  std::vector<void*> generatorShuffleBindings(
      generatorShuffleEngine->getNbBindings());
  ...
  if (gFeedAttentionToInput) {
    genShuffleBindingMap["previous_output_attention"] =
        *outputAttentionDeviceBuffer;
    genShuffleBindingMap["input_attention"] = *inputAttentionDeviceBuffer;
  }
  processBindings(generatorShuffleBindings, genShuffleBindingMap,
                  generatorShuffleEngine);

  // Create Tensor RT contexts
  nvinfer1::IExecutionContext* encoderContext =
      encoderEngine->createExecutionContext();
  nvinfer1::IExecutionContext* generatorContext =
      generatorEngine->createExecutionContext();
  nvinfer1::IExecutionContext* generatorShuffleContext =
      generatorShuffleEngine->createExecutionContext();
  ...
  int inputSamplesRead = dataReader->read(
      gMaxBatchSize, gMaxInputSequenceLength, *inputOriginalHostBuffer,
      *inputOriginalSequenceLengthsHostBuffer);
  if (gEnableProfiling)
    profilers[0].reportLayerTime(
        "Data Read",
        std::chrono::duration<float, std::milli>(
            std::chrono::high_resolution_clock::now() - startDataRead)
            .count());
  ...
  while (inputSamplesRead > 0) {
    ...
    // The idea is that shorter input sequences gets translated faster so we can
    // reduce batch size quickly for the
    // generator
    auto startBatchSort = std::chrono::high_resolution_clock::now();
    std::vector<int> samplePositions(inputSamplesRead);
    {
      std::vector<std::pair<int, int>> sequenceSampleIdAndLength(
          inputSamplesRead);
      for (int sampleId = 0; sampleId < inputSamplesRead; ++sampleId)
        sequenceSampleIdAndLength[sampleId] = std::make_pair(
            sampleId,
            ((const int*)*inputOriginalSequenceLengthsHostBuffer)[sampleId]);
      std::sort(sequenceSampleIdAndLength.begin(),
      ...
      for (int position = 0; position < inputSamplesRead; ++position) {
        ...
        ((int*)*inputSequenceLengthsHostBuffer)[position] =
            ((const int*)*inputOriginalSequenceLengthsHostBuffer)[sampleId];
        std::copy_n(
            ((const int*)*inputOriginalHostBuffer) +
                sampleId * gMaxInputSequenceLength,
            gMaxInputSequenceLength,
    ...
    if (gEnableProfiling)
      profilers[0].reportLayerTime(
          "Intra-batch Sort",
          std::chrono::duration<float, std::milli>(
              std::chrono::high_resolution_clock::now() - startBatchSort)
              .count());

    CUDA_CHECK(cudaMemcpyAsync(
        *inputEncoderDeviceBuffer, *inputHostBuffer,
        inputSamplesRead * gMaxInputSequenceLength * sizeof(int),
        cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(
        *inputSequenceLengthsDeviceBuffer, *inputSequenceLengthsHostBuffer,
        inputSamplesRead * sizeof(int), cudaMemcpyHostToDevice, stream));

    // Overlap host and device: Read data for the next batch while encode for
    // this one is running
    std::future<int> nextInputSamplesReadFuture =
        std::async(std::launch::async, [&]() {
          return dataReader->read(gMaxBatchSize, gMaxInputSequenceLength,
                                  *inputOriginalHostBuffer,
                                  *inputOriginalSequenceLengthsHostBuffer);
        });

    encoderContext->enqueue(inputSamplesRead, &encoderBindings[0], stream,
                            nullptr);

    // Limit output sequences length to input_sequence_length * 2
    std::transform(
        (const int*)*inputSequenceLengthsHostBuffer,
        (const int*)*inputSequenceLengthsHostBuffer + inputSamplesRead,
        (int*)*maxOutputSequenceLengthsHostBuffer, [](int i) {
          int r = i * 2;
          if (gMaxOutputSequenceLength >= 0)
            r = std::min(r, gMaxOutputSequenceLength);
          return r;
        });
    searchPolicy->initialize(inputSamplesRead,
                             *maxOutputSequenceLengthsHostBuffer);
    int batchMaxOutputSequenceLength = *std::max_element(
        (int*)*maxOutputSequenceLengthsHostBuffer,
        (int*)*maxOutputSequenceLengthsHostBuffer + inputSamplesRead);
    ...
    for (int outputTimestep = 0;
         (outputTimestep < batchMaxOutputSequenceLength) &&
         (validSampleCount > 0);
         ++outputTimestep) {
      // Generator initialization and beam shuffling
      if (outputTimestep == 0) {
        generatorContext->enqueue(
            validSampleCount, &generatorBindingsFirstStep[0], stream, nullptr);
      } else {
        generatorShuffleContext->enqueue(
            validSampleCount, &generatorShuffleBindings[0], stream, nullptr);
        generatorContext->enqueue(validSampleCount, &generatorBindings[0],
                                  stream, nullptr);
      }

      CUDA_CHECK(cudaMemcpyAsync(*outputCombinedLikelihoodHostBuffer,
                                 *outputCombinedLikelihoodDeviceBuffer,
                                 validSampleCount * gBeamWidth * sizeof(float),
                                 cudaMemcpyDeviceToHost, stream));
      CUDA_CHECK(cudaMemcpyAsync(*outputVocabularyIndicesHostBuffer,
                                 *inputDecoderDeviceBuffer,
                                 validSampleCount * gBeamWidth * sizeof(int),
                                 cudaMemcpyDeviceToHost, stream));
      CUDA_CHECK(cudaMemcpyAsync(*outputRayOptionIndicesHostBuffer,
                                 *outputRayOptionIndicesDeviceBuffer,
                                 validSampleCount * gBeamWidth * sizeof(int),
                                 cudaMemcpyDeviceToHost, stream));
      ...
      searchPolicy->processTimestep(
          validSampleCount, *outputCombinedLikelihoodHostBuffer,
          *outputVocabularyIndicesHostBuffer, *outputRayOptionIndicesHostBuffer,
          *sourceRayIndicesHostBuffer, *sourceLikelihoodsHostBuffer);
      if (gEnableProfiling)
        profilers[0].reportLayerTime(
            "Beam Search",
            std::chrono::duration<float, std::milli>(
      ...
      CUDA_CHECK(cudaMemcpyAsync(*sourceRayIndicesDeviceBuffer,
                                 *sourceRayIndicesHostBuffer,
                                 validSampleCount * gBeamWidth * sizeof(int),
                                 cudaMemcpyHostToDevice, stream));
      CUDA_CHECK(cudaMemcpyAsync(*inputLikelihoodsDeviceBuffer,
    ...
    searchPolicy->readGeneratedResult(
        inputSamplesRead, batchMaxOutputSequenceLength, &outputHostBuffer[0],
        *outputSequenceLengthsHostBuffer);
    if (gEnableProfiling)
      profilers[0].reportLayerTime(
          "Read Result",
          std::chrono::duration<float, std::milli>(
              std::chrono::high_resolution_clock::now() - startBacktrack)
              .count());
    ...
    if (gEnableProfiling)
      profilers[0].reportLayerTime(
          "Data Write",
          std::chrono::duration<float, std::milli>(
              std::chrono::high_resolution_clock::now() - startDataWrite)
              .count());
  ...
  float totalLatency =
      std::chrono::duration<float, std::milli>(
          std::chrono::high_resolution_clock::now() - startLatency)
          .count();

  dataWriter->finalize();
  float score = gDataWriterStr == "bleu"
                    ? static_cast<nmtSample::BLEUScoreWriter*>(dataWriter.get())
                          ->getScore()
                    : -1.0f;

  if (gDataWriterStr == "benchmark") {
    gLogInfo << "Average latency (without data read) = "
             << totalLatency / static_cast<float>(batchCount) << " ms"
             << std::endl;
  }
  ...
  if (gEnableProfiling) {
    if (gAggregateProfiling) {
      ...
      for (const auto& profiler : profilers) gLogInfo << profiler << std::endl;
    }
  }
