...
//! This file contains the implementation of the FasterRCNN sample. It creates
//! the network using
//! the FasterRCNN caffe model.
//! It can be run with the following command line:
//! Command: ./sample_fasterRCNN [-h or --help] [-d=/path/to/data/dir or
...
//!
//! \brief The SampleFasterRCNNParams structure groups the additional parameters
//! required by
...
class SampleFasterRCNN {
 ...
 private:
  ...
  std::vector<samplesCommon::PPM<kIMG_CHANNELS, kIMG_H, kIMG_W>>
      mPPMs;  //!< PPMs of test images
  ...
  //!
  //! \brief Filters output detections, handles post-processing of bounding
  //! boxes and verify results
  //!
  ...
  //!
  void bboxTransformInvAndClip(const float* rois, const float* deltas,
                               float* predBBoxes, const float* imInfo,
                               const int N, const int nmsMaxOut,
                               const int numCls);
  ...
  std::vector<int> nonMaximumSuppression(
      std::vector<std::pair<float, int>>& scoreIndex, float* bbox,
...
//! engine
//!
//! \details This function creates the FasterRCNN network by parsing the caffe
//! model and builds
...
//!
void SampleFasterRCNN::constructNetwork(
...
//! a managed buffer
//!
bool SampleFasterRCNN::processInput(
  ...
  const std::vector<std::string> imageList = {
      "000456.ppm", "000542.ppm", "001150.ppm", "001763.ppm", "004545.ppm"};
  mPPMs.resize(batchSize);
  assert(mPPMs.size() <= imageList.size());

  // Fill im_info buffer
  float* hostImInfoBuffer =
      static_cast<float*>(buffers.getHostBuffer("im_info"));
  ...
  const float pixelMean[3]{102.9801f, 115.9465f,
                           122.7717f};  // Also in BGR order
  for (int i = 0, volImg = inputC * inputH * inputW; i < batchSize; ++i) {
    for (int c = 0; c < inputC; ++c) {
      // The color image to input should be in BGR order
      for (unsigned j = 0, volChl = inputH * inputW; j < volChl; ++j)
        hostDataBuffer[i * volImg + c * volChl + j] =
            float(mPPMs[i].buffer[j * inputC + 2 - c]) - pixelMean[c];
    }
  }
...
//!
//! \brief Filters output detections and handles post-processing of bounding
//! boxes, verify result
//!
//! \return whether the detection output matches expectations
//!
bool SampleFasterRCNN::verifyOutput(
    const samplesCommon::BufferManager& buffers) {
  const int batchSize = mParams.batchSize;
  ...
  const float* imInfo =
      static_cast<const float*>(buffers.getHostBuffer("im_info"));
  const float* deltas =
      static_cast<const float*>(buffers.getHostBuffer("bbox_pred"));
  const float* clsProbs =
      static_cast<const float*>(buffers.getHostBuffer("cls_prob"));
  ...
  bboxTransformInvAndClip(rois, deltas, predBBoxes.data(), imInfo, batchSize,
                          nmsMaxOut, outputClsSize);
  ...
  // The sample passes if there is at least one detection for each item in the
  // batch
  bool pass = true;
    ...
    for (int c = 1; c < outputClsSize; ++c)  // Skip the background
    {
      ...
      for (int r = 0; r < nmsMaxOut; ++r) {
        if (scores[r * outputClsSize + c] > score_threshold) {
          scoreIndex.push_back(
              std::make_pair(scores[r * outputClsSize + c], r));
      ...
      for (unsigned k = 0; k < indices.size(); ++k) {
        ...
        const std::string storeName =
            classes[c] + "-" + std::to_string(scores[idx * outputClsSize + c]) +
            ".ppm";
        gLogInfo << "Detected " << classes[c] << " in " << mPPMs[i].fileName
                 << " with confidence "
                 << scores[idx * outputClsSize + c] * 100.0f << "% "
                 << " (Result stored in " << storeName << ")." << std::endl;

        const samplesCommon::BBox b{bbox[idx * outputBBoxSize + c * 4],
                                    bbox[idx * outputBBoxSize + c * 4 + 1],
                                    bbox[idx * outputBBoxSize + c * 4 + 2],
                                    bbox[idx * outputBBoxSize + c * 4 + 3]};
...
void SampleFasterRCNN::bboxTransformInvAndClip(
  ...
  for (int i = 0; i < N * nmsMaxOut; ++i) {
    ...
    for (int j = 0; j < numCls; ++j) {
      ...
      predBBoxes[i * numCls * 4 + j * 4] = std::max(
          std::min(pred_ctr_x - 0.5f * pred_w, imInfo_offset[1] - 1.f), 0.f);
      predBBoxes[i * numCls * 4 + j * 4 + 1] = std::max(
          std::min(pred_ctr_y - 0.5f * pred_h, imInfo_offset[0] - 1.f), 0.f);
      predBBoxes[i * numCls * 4 + j * 4 + 2] = std::max(
          std::min(pred_ctr_x + 0.5f * pred_w, imInfo_offset[1] - 1.f), 0.f);
      predBBoxes[i * numCls * 4 + j * 4 + 3] = std::max(
          std::min(pred_ctr_y + 0.5f * pred_h, imInfo_offset[0] - 1.f), 0.f);
    }
  }
}
  ...
  for (auto i : scoreIndex) {
    ...
    for (unsigned k = 0; k < indices.size(); ++k) {
      if (keep) {
        ...
        float overlap =
            computeIoU(&bbox[(idx * numClasses + classNum) * 4],
...
SampleFasterRCNNParams initializeSampleParams(const samplesCommon::Args& args) {
  ...
  params.nmsMaxOut = 300;  // This value needs to be changed as per the
                           // nmsMaxOut value set in RPROI plugin parameters in
                           // prototxt
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_fasterRCNN [-h or --help] [-d or "
