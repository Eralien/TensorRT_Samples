...
//! Command: ./sample_uff_mnist [-h or --help] [-d or --datadir=<path to data
...
void SampleUffMNIST::constructNetwork(
  ...
  parser->parse(mParams.uffFileName.c_str(), *network,
...
//!
//! \brief Reads the input data, preprocesses, and stores the result in a
//! managed buffer
//!
...
//!  It allocates the buffer, sets inputs, executes the engine, and verifies the
//!  output.
//!
...
//!
samplesCommon::UffSampleParams initializeSampleParams(
    const samplesCommon::Args& args) {
  samplesCommon::UffSampleParams params;
  if (args.dataDirs
          .empty())  //!< Use default directories if user hasn't provided paths
  {
