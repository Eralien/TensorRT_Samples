...
//!
//! SampleINT8.cpp
//! This file contains the implementation of the sample. It creates the network
//! using
//! the caffe model.
//! It can be run with the following command line:
//! Command: ./sample_int8 [-h or --help] [-d=/path/to/data/dir or
...
//!
//! \brief The SampleINT8Params structure groups the additional parameters
//! required by
//!         the INT8 sample.
//!
...
class SampleINT8 {
 ...
 public:
  ...
  //!
  bool infer(std::pair<float, float>& score, int firstScoreBatch,
             int nbScoreBatches);
 ...
 private:
  ...
  //! \brief Scores model
  //!
  int calculateScore(const samplesCommon::BufferManager& buffers, float* labels,
                     int batchSize, int outputSize, int threshold);
};
...
//! engine
//!
//! \details This function creates the network by parsing the caffe model and
//! builds
...
bool SampleINT8::build(DataType dataType) {
  ...
  auto parser = SampleUniquePtr<nvcaffeparser1::ICaffeParser>(
  ...
  auto constructed =
      constructNetwork(builder, network, config, parser, dataType);
  if (!constructed) {
    return false;
  }
...
//!        output layers
//!
//! \param network Pointer to the network that will be populated with the
//! network
//!
//! \param builder Pointer to the engine builder
//!
bool SampleINT8::constructNetwork(
  ...
  if (dataType == DataType::kINT8) {
    MNISTBatchStream calibrationStream(
        mParams.calBatchSize, mParams.nbCalBatches, "train-images-idx3-ubyte",
        "train-labels-idx1-ubyte", mParams.dataDirs);
    calibrator.reset(new Int8EntropyCalibrator2<MNISTBatchStream>(
        calibrationStream, 0, mParams.networkName.c_str(),
  ...
  if (mParams.dlaCore >= 0) {
    ...
    if (mParams.batchSize > builder->getMaxDLABatchSize()) {
      gLogError << "Requested batch size " << mParams.batchSize
...
//!
bool SampleINT8::infer(std::pair<float, float>& score, int firstScoreBatch,
                       int nbScoreBatches) {
  float ms{0.0f};
  ...
  MNISTBatchStream batchStream(
      mParams.batchSize, nbScoreBatches + firstScoreBatch,
      "train-images-idx3-ubyte", "train-labels-idx1-ubyte", mParams.dataDirs);
  batchStream.skip(firstScoreBatch);

  Dims outputDims = context->getEngine().getBindingDimensions(
      context->getEngine().getBindingIndex(
          mParams.outputTensorNames[0].c_str()));
  ...
  while (batchStream.next()) {
    ...
    bool status = context->enqueue(
        mParams.batchSize, buffers.getDeviceBindings().data(), stream, nullptr);
    if (!status) {
      return false;
    }
    ...
    top1 += calculateScore(buffers, batchStream.getLabels(), mParams.batchSize,
                           outputSize, 1);
    top5 += calculateScore(buffers, batchStream.getLabels(), mParams.batchSize,
                           outputSize, 5);
  ...
  int imagesRead =
      (batchStream.getBatchesRead() - firstScoreBatch) * mParams.batchSize;
  score.first = float(top1) / float(imagesRead);
  score.second = float(top5) / float(imagesRead);

  gLogInfo << "Top1: " << score.first << ", Top5: " << score.second
           << std::endl;
  gLogInfo << "Processing " << imagesRead << " images averaged "
           << totalTime / imagesRead << " ms/image and "
           << totalTime / batchStream.getBatchesRead() << " ms/batch."
           << std::endl;
...
int SampleINT8::calculateScore(const samplesCommon::BufferManager& buffers,
                               float* labels, int batchSize, int outputSize,
                               int threshold) {
  float* probs =
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_int8 [-h or --help] [-d or --datadir=<path to "
               "data directory>] "
               "[--useDLACore=<int>]"
            << std::endl;
  ...
  std::cout << "start=N         Set the first batch to be scored (default = "
               "100). All batches before this batch will "
               "be used for calibration."
            << std::endl;
  std::cout << "score=N         Set the number of batches to be scored "
               "(default = 400)."
            << std::endl;
}
...
int main(int argc, char** argv) {
  if (argc >= 2 &&
      (!strncmp(argv[1], "help", 4) || !strncmp(argv[1], "--help", 6) ||
       !strncmp(argv[1], "--h", 3) || !strncmp(argv[1], "-h", 2))) {
    printHelpInfo();
    return EXIT_FAILURE;
  }

  // By default we score over 57600 images starting at 512, so we don't score
  // those used to search calibration
  int batchSize = 32;
  ...
  gLogInfo << "Building and running a GPU inference engine for INT8 sample"
           << std::endl;

  std::vector<std::string> dataTypeNames = {"FP32", "FP16", "INT8"};
  std::vector<DataType> dataTypes = {DataType::kFLOAT, DataType::kHALF,
                                     DataType::kINT8};
  std::vector<std::pair<float, float>> scores(3, std::make_pair(0.0f, 0.0f));
  for (size_t i = 0; i < dataTypes.size(); i++) {
    gLogInfo << dataTypeNames[i] << " run:" << nbScoreBatches
             << " batches of size " << batchSize << " starting at "
             << firstScoreBatch << std::endl;

    if (!sample.build(dataTypes[i])) {
      if (!sample.isSupported(dataTypes[i])) {
        gLogWarning << "Skipping " << dataTypeNames[i]
                    << " since the platform does not support this data type."
                    << std::endl;
        continue;
      }
  ...
  double fp16tolerance{0.01}, int8tolerance{0.01};

  if (scores[1].first != 0.0f &&
      !isApproximatelyEqual(scores[0].first, scores[1].first, fp16tolerance)) {
    gLogError << "FP32(" << scores[0].first << ") and FP16(" << scores[1].first
              << ") Top1 accuracy differ by more than " << fp16tolerance << "."
              << std::endl;
    return gLogger.reportFail(sampleTest);
  }
  if (scores[2].first != 0.0f &&
      !isApproximatelyEqual(scores[0].first, scores[2].first, int8tolerance)) {
    gLogError << "FP32(" << scores[0].first << ") and Int8(" << scores[2].first
              << ") Top1 accuracy differ by more than " << int8tolerance << "."
              << std::endl;
    return gLogger.reportFail(sampleTest);
  }
  if (scores[1].second != 0.0f &&
      !isApproximatelyEqual(scores[0].second, scores[1].second,
                            fp16tolerance)) {
    gLogError << "FP32(" << scores[0].second << ") and FP16("
              << scores[1].second << ") Top5 accuracy differ by more than "
              << fp16tolerance << "." << std::endl;
    return gLogger.reportFail(sampleTest);
  }
  if (scores[2].second != 0.0f &&
      !isApproximatelyEqual(scores[0].second, scores[2].second,
                            int8tolerance)) {
    gLogError << "FP32(" << scores[0].second << ") and INT8("
              << scores[2].second << ") Top5 accuracy differ by more than "
              << int8tolerance << "." << std::endl;
    return gLogger.reportFail(sampleTest);
  }
