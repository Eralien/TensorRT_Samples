...
//! sampleINT8API.cpp
//! This file contains implementation showcasing usage of INT8 calibration and
//! precision APIs.
//! It creates classification networks such as mobilenet, vgg19, resnet-50 from
//! onnx model file.
//! This sample showcae setting per tensor dynamic range overriding calibrator
//! generated scales if it exists.
//! This sample showcase how to set computation precision of layer. It involves
//! forcing output tensor type of the layer
//! to particular precision. It can be run with the following command line:
//! Command: ./sample_int8_api [-h or --help]
//! [-m modelfile] [-s per_tensor_dynamic_range_file] [-i image_file] [-r
//! reference_file] [-d path/to/data/dir]
//! [--verbose] [-useDLA <id>]
...
//!
//! \brief The SampleINT8APIParams structure groups the additional parameters
//! required by
//!         the INT8 API sample
//!
...
//!
//! \brief The SampleINT8API class implements INT8 inference on classification
//! networks.
//!
//! \details INT8 API usage for setting custom int8 range for each input layer.
//! API showcase how
...
class SampleINT8API {
 ...
 private:
  ...
  std::map<std::string, std::string>
      mInOut;  //!< Input and output mapping of the network
  ...
  void
  getInputOutputNames();  //!< Populates input and output mapping of the network

  //!
  //! \brief Reads the ppm input image, preprocesses, and stores the result in a
  //! managed buffer
  //!
  ...
  //!
  void writeNetworkTensorNames(
      const SampleUniquePtr<nvinfer1::INetworkDefinition>& network);
};
...
void SampleINT8API::getInputOutputNames() {
  ...
  for (int b = 0; b < nbindings; ++b) {
    ...
    if (mEngine.get()->bindingIsInput(b)) {
      if (mParams.verbose) {
        gLogInfo << "Found input: " << mEngine.get()->getBindingName(b)
                 << " shape=" << dims
      ...
      if (mParams.verbose) {
        gLogInfo << "Found output: " << mEngine.get()->getBindingName(b)
                 << " shape=" << dims
                 << " dtype=" << (int)mEngine.get()->getBindingDataType(b)
                 << std::endl;
      }
...
bool SampleINT8API::readPerTensorDynamicRangeValues() {
  ...
  if (!iDynamicRangeStream) {
    gLogError << "Could not find per tensor scales file: "
              << mParams.dynamicRangeFileName << std::endl;
    return false;
  }
...
//! \brief  Sets computation precision for network layers
//!
void SampleINT8API::setLayerPrecision(
    SampleUniquePtr<nvinfer1::INetworkDefinition>& network) {
  ...
  for (int i = 0; i < network->getNbLayers(); ++i) {
    ...
    for (int j = 0; j < layer->getNbOutputs(); ++j) {
      ...
      if (mParams.verbose) {
        std::string tensorName = layer->getOutput(j)->getName();
        gLogInfo << "Tensor: " << tensorName << ". OutputType: INT8"
                 << std::endl;
      }
...
//! \brief  Write network tensor names to a file.
//!
void SampleINT8API::writeNetworkTensorNames(
    const SampleUniquePtr<nvinfer1::INetworkDefinition>& network) {
  gLogInfo << "Sample requires to run with per tensor dynamic range."
           << std::endl;
  gLogInfo << "In order to run Int8 inference without calibration, user will "
              "need to provide dynamic range for all "
              "the network tensors."
           << std::endl;
  ...
  tensorsFile.close();
  gLogInfo << "Successfully generated network tensor names. Writing: "
           << mParams.networkTensorsFileName << std::endl;
  gLogInfo << "Use the generated tensor names file to create dynamic range "
              "file for Int8 inference. Follow README.md "
...
//! \brief  Sets custom dynamic range for network tensors
//!
bool SampleINT8API::setDynamicRange(
    SampleUniquePtr<nvinfer1::INetworkDefinition>& network) {
  ...
  if (mParams.verbose) {
    gLogInfo << "If dynamic range for a tensor is missing, TensorRT will run "
                "inference assuming dynamic range for "
                "the tensor as optional."
             << std::endl;
    gLogInfo << "If dynamic range for a tensor is required then inference will "
                "fail. Follow README.md to generate "
  ...
  for (int i = 0; i < network->getNbInputs(); ++i) {
    string tName = network->getInput(i)->getName();
    if (mPerTensorDynamicRangeMap.find(tName) !=
  ...
  for (int i = 0; i < network->getNbLayers(); ++i) {
    ...
    for (int j = 0, e = lyr->getNbOutputs(); j < e; ++j) {
      ...
      if (mPerTensorDynamicRangeMap.find(tName) !=
          mPerTensorDynamicRangeMap.end()) {
        // Calibrator generated dynamic range for network tensor can be
        // overriden or set using below API
        lyr->getOutput(j)->setDynamicRange(-mPerTensorDynamicRangeMap.at(tName),
                                           mPerTensorDynamicRangeMap.at(tName));
      } else if (lyr->getType() == LayerType::kCONSTANT) {
        IConstantLayer* cLyr = static_cast<IConstantLayer*>(lyr);
        if (mParams.verbose) {
          gLogWarning << "Computing missing dynamic range for tensor, " << tName
                      << ", from weights." << std::endl;
        }
        ...
        for (int64_t wb = 0, we = wts.count; wb < we; ++wb) {
          ...
          switch (wts.type) {
            ...
            case DataType::kINT8:
              ...
              break;
            case DataType::kHALF:
              val = static_cast<const half_float::half*>(wts.values)[wb];
              break;
  ...
  if (mParams.verbose) {
    ...
    for (auto iter = mPerTensorDynamicRangeMap.begin();
         iter != mPerTensorDynamicRangeMap.end(); ++iter)
      gLogInfo << "Tensor: " << iter->first
               << ". Max Absolute Dynamic Range: " << iter->second << std::endl;
  }
  return true;
}
...
bool SampleINT8API::prepareInput(const samplesCommon::BufferManager& buffers) {
  if (samplesCommon::toLower(samplesCommon::getFileType(mParams.imageFileName))
          .compare("ppm") != 0) {
    gLogError << "Wrong format: " << mParams.imageFileName
  ...
  float* hostInputBuffer =
      static_cast<float*>(buffers.getHostBuffer(mInOut["input"]));
  ...
  for (int c = 0; c < channels; ++c) {
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        // 2. Normalize Image using per channel Mean and per channel Standard
        // Deviation
        // 3. Shuffle HWC to CHW form
        hostInputBuffer[dstIdx] =
            (2.0 / 255.0) * static_cast<const float>(fileData[srcIdx]) - 1.0;
      }
    }
  }
  return true;
...
//! \brief Verifies that the output is correct and prints it
//!
bool SampleINT8API::verifyOutput(
    const samplesCommon::BufferManager& buffers) const {
  // copy output host buffer data for further processing
  const float* probPtr =
      static_cast<const float*>(buffers.getHostBuffer(mInOut.at("output")));
  ...
  if (!samplesCommon::readReferenceFile(mParams.referenceFileName,
                                        referenceVector)) {
    gLogError << "Unable to read reference file: " << mParams.referenceFileName
              << std::endl;
    return false;
  }

  vector<string> top5Result =
      samplesCommon::classify(referenceVector, output, 5);
...
//! engine
//!
//! \details This function creates INT8 classification network by parsing the
//! onnx model and builds
...
Logger::TestResult SampleINT8API::build() {
  ...
  if (!network) {
    gLogError << "Unable to create network object." << mParams.referenceFileName
              << std::endl;
    return Logger::TestResult::kFAILED;
  }
  ...
  if (!config) {
    gLogError << "Unable to create config object." << mParams.referenceFileName
              << std::endl;
    return Logger::TestResult::kFAILED;
  }
  ...
  if (!parser) {
    gLogError << "Unable to create parser object." << mParams.referenceFileName
              << std::endl;
    return Logger::TestResult::kFAILED;
  }
  ...
  if (!parser->parseFromFile(mParams.modelFileName.c_str(), verbosity)) {
    gLogError << "Unable to parse ONNX model file: " << mParams.modelFileName
              << std::endl;
    return Logger::TestResult::kFAILED;
  }
  ...
  if (!builder->platformHasFastInt8()) {
    gLogError << "Platform does not support INT8 inference. sampleINT8API can "
                 "only run in INT8 Mode."
              << std::endl;
    return Logger::TestResult::kWAIVED;
  }
  ...
  // Mark calibrator as null. As user provides dynamic range for each tensor, no
  // calibrator is required
  ...
  if (mParams.dlaCore >= 0) {
    ...
    if (maxBatchSize > builder->getMaxDLABatchSize()) {
      std::cerr << "Requested batch size " << maxBatchSize
                << " is greater than the max DLA batch size of "
                << builder->getMaxDLABatchSize()
  ...
  const int inputIndex =
      mEngine.get()->getBindingIndex(mInOut["input"].c_str());
  mInputDims = mEngine.get()->getBindingDimensions(inputIndex);

  const int outputIndex =
      mEngine.get()->getBindingIndex(mInOut["output"].c_str());
...
//! allocates
//!          the buffer, sets inputs, executes the engine, and verifies the
//!          output
//!
Logger::TestResult SampleINT8API::infer() {
  ...
  if (!context->enqueueV2(buffers.getDeviceBindings().data(), stream,
                          nullptr)) {
    return Logger::TestResult::kFAILED;
  }
  ...
  return verifyOutput(buffers) ? Logger::TestResult::kRUNNING
...
//!
//! \brief The SampleINT8APIArgs structures groups the additional arguments
//! required by
//!         the INT8 API sample
//!
...
void validateInputParams(SampleINT8APIParams& params) {
  gLogInfo << "Please follow README.md to generate missing input files."
           << std::endl;
  gLogInfo << "Validating input parameters. Using following input files for "
              "inference."
           << std::endl;
  ...
  if (params.writeNetworkTensors) {
    gLogInfo << "    Writing Network Tensors File to: "
             << params.networkTensorsFileName << std::endl;
    return;
  }
  ...
  params.referenceFileName =
      locateFile(params.referenceFileName, params.dataDirs);
  gLogInfo << "    Reference File: " << params.referenceFileName << std::endl;
  params.dynamicRangeFileName =
      locateFile(params.dynamicRangeFileName, params.dataDirs);
  gLogInfo << "    Dynamic Range File: " << params.dynamicRangeFileName
           << std::endl;
  return;
}
...
void printHelpInfo() {
  std::cout << "Usage: ./sample_int8_api [-h or --help] [--model=<model file>] "
               "[--ranges=<dynamic range file>] [--image=<image file>] "
               "[--reference=<reference file>] "
               "[--data=<path to data directory>] [--useDLACore=<int>] [-v or "
               "--verbose]\n";
  std::cout << "--help, -h                        Display This help information"
            << std::endl;
  std::cout << "--model=<model file>              Path to the model file "
               "(default = resnet50.onnx)"
            << std::endl;
  std::cout << "--image=<image file>              Path to the image file to "
               "infer (default = airlines.ppm)"
            << std::endl;
  std::cout << "--reference=<reference file>      Path to the reference labels "
               "file (default = reference_labels.txt)"
            << std::endl;
  std::cout << "--ranges=<dynamic range file>     Path to the custom per "
               "tensor dynamic range file for the network "
               "(default = resnet50_per_tensor_dynamic_range.txt)"
            << std::endl;
  std::cout << "--write_tensors                   Option to generate file "
               "containing network tensors name. By default writes to "
               "network_tensors.txt file. To provide user defined file name "
               "use additional option "
               "--network_tensors_file. See --network_tensors_file option "
               "usage for more detail."
            << std::endl;
  std::cout << "--network_tensors_file=<file>     Path to the network tensors "
               "file. This option "
               "needs to be used with --write_tensors option. Specify file "
               "name (will write to current execution "
               "directory) or absolute path to file name to write network "
               "tensor names file. Dynamic range "
               "corresponding to each network tensor is required to run the "
               "sample (default = network_tensors.txt)"
            << std::endl;
  std::cout << "--data=<path to data dir>         Path to the data directory "
               "to search for above files in case absolute paths to "
               "files are not provided (default both data/samples/int8_api/ "
               "and data/int8_api/)"
            << std::endl;
  std::cout << "--useDLACore=N                    Specify a DLA engine for "
               "layers that support DLA. Value can range from 0 to n-1, "
...
int main(int argc, char** argv) {
  ...
  gLogInfo << "Building and running a INT8 GPU inference engine for "
           << params.modelFileName << std::endl;
