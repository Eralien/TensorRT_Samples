...
//!
//! It builds a TensorRT engine by importing a trained MNIST Caffe model, and
//! replaces the final
//! Fully Connected (FC) layer with a custom plugin layer. It uses the engine to
//! run inference on an input image of a
//! digit. It can be run with the following command line: Command:
//! ./sample_plugin [-h or --help] [-d=/path/to/data/dir
...
//!
//! \details It creates the network using a trained Caffe MNIST classification
//! model, and replaces the
...
//! engine
//!
//! \details This function creates the MNIST network by parsing the caffe model
//! and builds
...
bool SamplePlugin::build() {
  ...
  // The PluginFactory object contains the methods to construct the FC plugin
  // layer
  ...
  // deserialize mModelStream with a IRuntime object to recreate the original
  // engine.
  // Note for this sample we could have simply used the original engine produced
  // by builder->buildEngineWithConfig()
  auto modelStream = SampleUniquePtr<nvinfer1::IHostMemory>(
      builder->buildEngineWithConfig(*network, *config)->serialize());
  ...
  mEngine = std::shared_ptr<nvinfer1::ICudaEngine>(
      runtime->deserializeCudaEngine(modelStream->data(), modelStream->size(),
                                     &pluginFactory),
...
//!
void SamplePlugin::constructNetwork(
  ...
  const nvcaffeparser1::IBlobNameToTensor* blobNameToTensor =
      parser->parse(mParams.prototxtFileName.c_str(),
                    mParams.weightsFileName.c_str(), *network, dType);
...
bool SamplePlugin::infer() {
  ...
  if (!context->enqueue(mParams.batchSize, buffers.getDeviceBindings().data(),
                        stream, nullptr)) {
    return false;
  }
  ...
  bool outputCorrect =
      verifyOutput(buffers, mParams.outputTensorNames[0], digit);

  // The output correctness is not used to determine the test result.
  if (!outputCorrect && mParams.dlaCore != -1) {
    gLogInfo << "Warning: infer result is not correct. It maybe caused by "
                "dummy scales in INT8 mode."
             << std::endl;
  }
...
bool SamplePlugin::processInput(const samplesCommon::BufferManager& buffers,
  ...
  float* hostInputBuffer =
      static_cast<float*>(buffers.getHostBuffer(inputTensorName));
...
bool SamplePlugin::verifyOutput(const samplesCommon::BufferManager& buffers,
                                ...
                                int groundTruthDigit) const {
  const float* prob =
      static_cast<const float*>(buffers.getHostBuffer(outputTensorName));
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_plugin [-h or --help] [-d or --datadir=<path "
               "to data directory>] [--useDLACore=<int>]\n";
...
int main(int argc, char** argv) {
  ...
  gLogInfo << "Building and running a GPU inference engine for MNIST"
           << std::endl;
