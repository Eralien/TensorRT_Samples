...
class FCPlugin : public nvinfer1::IPluginExt {
 public:
  FCPlugin(const nvinfer1::Weights* weights, int nbWeights,
    ...
    assert(mKernelWeights.type == nvinfer1::DataType::kFLOAT ||
           mKernelWeights.type == nvinfer1::DataType::kHALF);

    mBiasWeights = weights[1];
    assert(mBiasWeights.count == 0 || mBiasWeights.count == nbOutputChannels);
    assert(mBiasWeights.type == nvinfer1::DataType::kFLOAT ||
           mBiasWeights.type == nvinfer1::DataType::kHALF);

    mKernelWeights.values =
        malloc(mKernelWeights.count * type2size(mKernelWeights.type));
    std::memcpy(const_cast<void*>(mKernelWeights.values), weights[0].values,
                mKernelWeights.count * type2size(mKernelWeights.type));
    mBiasWeights.values =
        malloc(mBiasWeights.count * type2size(mBiasWeights.type));
  ...
  bool supportsFormat(nvinfer1::DataType type,
    ...
    return (type == nvinfer1::DataType::kFLOAT ||
            (type == nvinfer1::DataType::kHALF && smVersion >= 0x600)) &&
  ...
  int initialize() override {
    ...
    CHECK(cudnnCreateTensorDescriptor(&mSrcDescriptor));  // create cudnn tensor
                                                          // descriptors we need
                                                          // for bias addition
  ...
  virtual int enqueue(int batchSize, const void* const* inputs, void** outputs,
    ...
    if (mDataType == nvinfer1::DataType::kFLOAT) {
      CHECK(cublasSgemm(
          mCublas, CUBLAS_OP_T, CUBLAS_OP_N, mNbOutputChannels, batchSize,
    ...
    if (mBiasWeights.count) {
      cudnnDataType_t cudnnDT = mDataType == nvinfer1::DataType::kFLOAT
                                    ? CUDNN_DATA_FLOAT
                                    : CUDNN_DATA_HALF;
      CHECK(cudnnSetTensor4dDescriptor(mSrcDescriptor, CUDNN_TENSOR_NCHW,
                                       cudnnDT, 1, mNbOutputChannels, 1, 1));
      CHECK(cudnnSetTensor4dDescriptor(mDstDescriptor, CUDNN_TENSOR_NCHW,
                                       cudnnDT, batchSize, mNbOutputChannels, 1,
                                       1));
      CHECK(cudnnAddTensor(mCudnn, &onef, mSrcDescriptor, mDeviceBias, &onef,
                           mDstDescriptor, outputs[0]));
    }
 ...
 private:
  ...
  void convertAndCopyToDevice(void*& deviceWeights,
                              const nvinfer1::Weights& weights) {
    if (weights.type != mDataType)  // Weights are converted in host memory
                                    // first, if the type does not match
    {
      size_t size = weights.count * (mDataType == nvinfer1::DataType::kFLOAT
                                         ? sizeof(float)
                                         : sizeof(__half));
      ...
      deviceWeights =
          copyToDevice(weights.values, weights.count * type2size(mDataType));
    }
  }
  ...
  void deserializeToDevice(const char*& hostBuffer, void*& deviceWeights,
                           size_t size) {
...
class PluginFactory : public nvinfer1::IPluginFactory,
 ...
 public:
  ...
  virtual IPlugin* createPlugin(const char* layerName,
                                const nvinfer1::Weights* weights,
                                int nbWeights) override {
    try {
      // there's no way to pass parameters through from the model definition, so
      ...
      mPlugin = std::unique_ptr<FCPlugin>(
          new FCPlugin(weights, nbWeights, NB_OUTPUT_CHANNELS));
      return mPlugin.get();
