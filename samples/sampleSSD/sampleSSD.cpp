...
//!
//! sampleSSD.cpp
//! This file contains the implementation of the SSD sample. It creates the
//! network using
//! the SSD caffe model.
//! It can be run with the following command line:
//! Command: ./sample_ssd [-h or --help] [-d=/path/to/data/dir or
...
//!
//! \brief The SampleSSDParams structure groups the additional parameters
//! required by
//!         the SSD sample.
//!
...
//! engine
//!
//! \details This function creates the SSD network by parsing the caffe model
//! and builds
...
//!
bool SampleSSD::constructNetwork(
  ...
  if (mParams.int8) {
    ...
    BatchStream calibrationStream(mParams.batchSize, mParams.nbCalBatches,
                                  mParams.calibrationBatches, mParams.dataDirs);
    calibrator.reset(new Int8EntropyCalibrator2<BatchStream>(
        calibrationStream, 0, "SSD", mParams.inputTensorNames[0].c_str()));
...
bool SampleSSD::verifyOutput(const samplesCommon::BufferManager& buffers) {
  ...
  const float* detectionOut =
      static_cast<const float*>(buffers.getHostBuffer("detection_out"));
  const int* keepCount =
      static_cast<const int*>(buffers.getHostBuffer("keep_count"));
  ...
  for (int p = 0; p < batchSize; ++p) {
    ...
    for (int i = 0; i < keepCount[p]; ++i) {
      ...
      std::string storeName =
          classes[(int)det[1]] + "-" + std::to_string(det[2]) + ".ppm";
      ...
      gLogInfo << " Image name:" << mPPMs[p].fileName.c_str()
               << ", Label: " << classes[(int)det[1]].c_str() << ","
               << " confidence: " << det[2] * 100.f
               << " xmin: " << det[3] * inputW << " ymin: " << det[4] * inputH
               << " xmax: " << det[5] * inputW << " ymax: " << det[6] * inputH
               << std::endl;
...
SampleSSDParams initializeSampleParams(const samplesCommon::Args& args) {
  ...
  params.keepTopK = 200;  // Number of total bboxes to be kept per image after
                          // NMS step. It is same as
...
//!
void printHelpInfo() {
  std::cout << "Usage: ./sample_ssd [-h or --help] [-d or --datadir=<path to "
...
int main(int argc, char** argv) {
  ...
  gLogInfo << "Building and running a GPU inference engine for SSD"
           << std::endl;
